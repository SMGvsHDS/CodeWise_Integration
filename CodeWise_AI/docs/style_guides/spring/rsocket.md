...
Reactive Streams
semantics across network boundary — for streaming requests such as
Request-Stream
and
Channel
...
The responder may then return
PAYLOAD
frames with response messages, and in the case
of
REQUEST_CHANNEL
the requester may also send
PAYLOAD
frames with more request
messages.
When a request involves a stream of messages such as
Request-Stream
and
Channel
,
the responder must respect demand signals from the requester. Demand is expressed as a
number of messages. Initial demand is specified in
REQUEST_STREAM
and
REQUEST_CHANNEL
frames. Subsequent demand is signaled via
REQUEST_N
frames.
Each side may also send metadata notifications, via the
METADATA_PUSH
frame, that do not
...
The
Java implementation
for RSocket is built on
Project Reactor
. The transports for  TCP and WebSocket are
built on
Reactor Netty
. As a Reactive Streams
library, Reactor simplifies the job of implementing the protocol. For applications it is
a natural fit to use
Flux
and
Mono
...
The main contract
io.rsocket.RSocket
models the four request interaction types with
Mono
representing a promise for a
single message,
Flux
a stream of messages, and
io.rsocket.Payload
the actual
message with access to data and metadata as byte buffers. The
RSocket
contract is used
symmetrically. For requesting, the application is given an
RSocket
to perform
requests with. For responding, the application implements
RSocket
to handle requests.
...
RSocket Interface
— RSocket service declaration
as Java interface with
@RSocketExchange
...
support and auto-configuration for an
...
sending an RSocket
SETUP
frame with connection settings.
RSocketRequester
provides a
builder that helps to prepare an
...
Kotlin
```
RSocketRequester requester = RSocketRequester.builder().tcp(
"localhost"
,
7000
);
URI url = URI.create(
"https://example.org:8080/rsocket"
);
...
```
val
requester = RSocketRequester.builder().tcp(
"localhost"
,
7000
)
...
val
requester = RSocketRequester.builder().webSocket(url)
Copied!
```
...
Connection Setup
RSocketRequester.Builder
provides the following to customize the initial
SETUP
frame:
...
setupData(Object)
— data to include in the
SETUP
.
setupRoute(String, Object…​)
— route in the metadata to include in the
SETUP
.
setupMetadata(Object, MimeType)
— other metadata to include in the
SETUP
.
For data, the default mime type is derived from the first configured
Decoder
. For
...
handle the start of a connection and the content of the
SETUP
frame. Metadata may be
...
metadata values. By default only the basic codecs from
spring-core
for
String
,
...
new
Jackson2CborEncoder()))
.decoders(decoders -> decoders.add(
new
Jackson2CborDecoder()))
.build();
...
```
RSocketStrategies
is designed for re-use. In some scenarios, for example, client and server in
...
RSocketMessageHandler.responder(strategies,
new
ClientHandler());
(
2
...
, if
spring-web
is present, for efficient
route matching.
2
Create a responder from a class with
@MessageMapping
and/or
@ConnectMapping
methods.
...
RSocketMessageHandler handler = context.getBean(RSocketMessageHandler
.
class
)
;
...
```
import
org.springframework.beans.factory.getBean
val
context: ApplicationContext = ...
val
handler = context.getBean<RSocketMessageHandler>()
val
requester = RSocketRequester.builder()
...
Copied!
```
For the above you may also need to use
setHandlerPredicate
in
RSocketMessageHandler
to
...
See also
Annotated Responders
, for more on the programming model.
Advanced
...
mind that
@ConnectMapping
methods are essentially handlers of the
SETUP
frame which
...
2
Perform handling and return completion
Mono<Void>
.
```
@ConnectMapping
...
(
"5"
).retrieveFlow<StatusReport>().collect {
(
1
)
// ...
...
```
ViewBox viewBox = ... ;
Flux<AirportLocation> locations = requester.route(
"locate.radars.within"
)
(
1
)
...
The
data(Object)
method also accepts any Reactive Streams
Publisher
, including
Flux
and
...
Flux
which produces the
same types of values, consider using one of the overloaded
data
...
Kotlin
```
Mono<AirportLocation> location = requester.route(
"find.radar.EWR"
))
...
```
import
org.springframework.messaging.rsocket.retrieveAndAwait
val
location = requester.route(
"find.radar.EWR"
)
...
composite metadata
(the default) and if the
values are supported by a registered
Encoder
. For example:
Java
Kotlin
...
```
import
org.springframework.messaging.rsocket.retrieveFlow
val
...
return value.
Annotated Responders
RSocket responders can be implemented as
@MessageMapping
and
@ConnectMapping
methods.
@MessageMapping
methods handle individual requests while
@ConnectMapping
methods handle
...
Server Responders
To use annotated responders on the server side, add
RSocketMessageHandler
to your Spring
...
handler = context.getBean<RSocketMessageHandler>()
val
server = RSocketServer.create(handler.responder())
.bind(TcpServerTransport.create(
"localhost"
,
7000
...
and
routing
metadata by default. You can set its
MetadataExtractor
if you need to switch to a
...
formats to support. You’ll likely need the
spring-web
module for codec implementations.
By default
SimpleRouteMatcher
is used for matching routes via
AntPathMatcher
.
We recommend plugging in the
PathPatternRouteMatcher
from
spring-web
for
...
or
client
responder configuration is in place,
@MessageMapping
...
.
Note:
Use of the annotation is optional. A method argument that is not a simple type
...
.
@Header
Metadata value registered for extraction as described in
MetadataExtractor
.
@Headers Map<String, Object>
All metadata values registered for extraction as described in
MetadataExtractor
.
The return value is expected to be one or more Objects to be serialized as response
payloads. That can be asynchronous types like
Mono
or
Flux
, a concrete value, or
either
void
or a no-value asynchronous type such as
Mono<Void>
.
The RSocket interaction type that an
@MessageMapping
method supports is determined from
the cardinality of the input (i.e.
@Payload
...
.
0
For input this means the method does not have an
@Payload
argument.
For output this is
void
...
RSocket Interface
and can be used as a requester via
...
Copied!
```
There some differences between
@RSocketExhange
and
@MessageMapping
since the
...
any subsequent metadata push notifications through the
METADATA_PUSH
frame, i.e.
...
@MessageMapping
but based on metadata and data from the
SETUP
and
METADATA_PUSH
frames.
...
@ConnectMapping
methods cannot return data and must be declared with
void
or
Mono<Void>
...
name-value pairs that can then be accessed like headers by name, for example via
@Header
...
new
DefaultMetadataExtractor(metadataDecoders);
...
```
import
org.springframework.messaging.rsocket.metadataToExtract
val
extractor = DefaultMetadataExtractor(metadataDecoders)
extractor.metadataToExtract<Foo>(fooMimeType,
"foo"
)
Copied!
```
...
MimeType.valueOf(
"application/vnd.myapp.metadata+json"
),
new
...
val
extractor = DefaultMetadataExtractor(metadataDecoders)
extractor.metadataToExtract<Map<String, String>>(MimeType.valueOf(
"application/vnd.myapp.metadata+json"
)) { jsonMap, outputMap ->
outputMap.putAll(jsonMap)
}
...
registry.metadataToExtract<Foo>(fooMimeType,
"foo"
)
// ...
}
...
@RSocketExchange
methods. You can pass such an interface to
RSocketServiceProxyFactory
...
Start by creating the interface with
@RSocketExchange
methods:
```
interface
...
RadarService service = factory.createClient(RadarService
.
class
)
;
...
This variable can be a String or any Object, which is then formatted via
toString()
.
@Payload
...
via
ReactiveAdapterRegistry
. A payload must be provided unless the
required
attribute
is set to
false
...
Object
as long
as the next argument is the metadata entry
MimeType
...
any producer of value(s) that can be adapted to a Reactive Streams
Publisher
via
ReactiveAdapterRegistry
.
By default, the behavior of RSocket service methods with synchronous (blocking) method
signature depends on response timeout settings of the underlying RSocket
ClientTransport
as well as RSocket keep-alive settings.
