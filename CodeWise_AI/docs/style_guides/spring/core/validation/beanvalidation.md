...
Consider the following example, which shows a simple
PersonForm
...
To associate a declaration with an implementation, each
@Constraint
annotation
...
@Constraint
(validatedBy=MyConstraintValidator
.
class
)
public
...
private
val
aDependency: Foo) : ConstraintValidator {
// ...
}
Copied!
```
...
new
MethodValidationPostProcessor();
processor.setAdaptConstraintViolations(
true
);
return
processor;
...
s which
group errors by method parameter, and each exposes a
MethodParameter
, the argument
value, and a list of
MessageSourceResolvable
...
and
"name"
and can be customized too. For example:
Properties
```
Size.person.name
=
Please, provide a {0} that is between {2} and {1} characters long
person.name
=
username
Copied!
...
Max.degrees
=
You cannot provide more than {1} {0}
Copied!
```
...
Errors
are
automatically added to the binder’s
BindingResult
.
...
. This is useful when
combining globally configured bean validation with a Spring
Validator
configured
