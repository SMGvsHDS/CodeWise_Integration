...
Copied!
```
The next example provides validation behavior for the
Person
class by implementing the
following two methods of the
...
and, in case of validation errors, registers those with the given
Errors
object.
Implementing a
Validator
...
logic for each nested class of object in its own
Validator
implementation. A simple
example of a “rich” object would be a
Customer
that is composed of two
String
properties (a first and a second name) and a complex
Address
object.
Address
objects
...
dependency-inject or instantiate an
AddressValidator
within your
CustomerValidator
,
...
if
(!addressValidator.supports(Address
.
class
))
{
...
private
val
addressValidator: Validator) : Validator {
init
{
if
(addressValidator ==
...
ValidationUtils.invokeValidator(
this
.addressValidator, customer.address, errors)
}
finally
{
errors.popNestedPath()
...
methods it offers can be found in the
javadoc
.
...
a simple
Errors
representation which can be inspected: typically calling
hasErrors()
or the new
failOnError
