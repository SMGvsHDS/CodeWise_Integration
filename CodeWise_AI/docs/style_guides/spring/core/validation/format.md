...
core.convert
is a
general-purpose type conversion system. It provides a unified
ConversionService
API as
well as a strongly typed
Converter
...
addition, both the Spring Expression Language (SpEL) and
DataBinder
use this system to
bind field values. For example, when SpEL needs to coerce a
Short
to a
Long
to
...
web or desktop application. In such environments, you typically convert from
String
to support the client postback process, as well as back to
String
to support the
view rendering process. In addition, you often need to localize
String
values. The more
general
core.convert
...
directly. To directly address them, Spring provides a convenient
Formatter
SPI that
provides a simple and robust alternative to
PropertyEditor
...
conversion logic — for example, for converting between a
java.util.Date
and a
Long
.
...
application) and need to parse and print localized field values. The
ConversionService
...
T
parse
(String clientValue, Locale locale)
throws
ParseException
;
}
...
. Implement the
print()
operation to print an instance of
T
for
display in the client locale. Implement the
parse()
operation to parse an instance of
T
from the formatted representation returned from the client locale. Your
Formatter
should throw a
ParseException
or an
...
care to ensure that your
Formatter
implementation is thread-safe.
The
format
subpackages provide several
Formatter
...
Duration
objects
in different styles defined in the
...
Copied!
```
The Spring team welcomes community-driven
Formatter
contributions. See
GitHub Issues
to contribute.
...
. The following
listing shows the definition of the
AnnotationFormatterFactory
interface:
```
package
...
static
final
Set<Class<?>> FIELD_TYPES = Set.of(Short
.
class
,
Integer
...
{
return
configureFormatterFrom(annotation, fieldType);
}
public
Parser<Number>
getParser
...
Copied!
```
To trigger formatting, you can annotate fields with
@NumberFormat
, as the following
example shows:
Java
...
to format
Duration
fields in ISO-8601 and simplified styles,
and
@DateTimeFormat
to format fields such as
java.util.Date
,
java.util.Calendar
,
...
annotation are formatted in a certain way. With a shared
FormatterRegistry
, you define
