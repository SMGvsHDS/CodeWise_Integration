...
⌘ + k
Variables
You can reference variables in an expression by using the
#variableName
syntax. Variables
...
dollar sign:
$
When setting a variable or root context object in the
EvaluationContext
, it is advised
that the type of the variable or root context object be
public
.
...
#root
variable is always
defined and refers to the root context object. Although
#this
...
parser.parseExpression(expression).getValue(context, List
.
class
)
;
...
// Evaluates to a list containing [11, 13, 17].
val
primesGreaterThanTen = parser.parseExpression(expression)
.getValue(context)
as
List<
Int
...
String expression =
"#root.inventions.![#root.name + ' invented the ' + #this + '.']"
;
...
// "Nikola Tesla invented the Tesla coil transformer."
val
results = parser.parseExpression(expression)
.getValue(context, tesla, List::
class
.
java
