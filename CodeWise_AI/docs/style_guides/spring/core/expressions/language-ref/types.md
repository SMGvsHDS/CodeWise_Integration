...
T
operator to specify an instance of
java.lang.Class
(the
...
T()
references to types within the
java.lang
...
Kotlin
```
Class dateClass = parser.parseExpression(
"T(java.util.Date)"
).getValue(Class
.
class
...
;
Class stringClass = parser.parseExpression(
"T(String)"
).getValue(Class
.
class
...
```
val
dateClass = parser.parseExpression(
"T(java.util.Date)"
).getValue(Class::
class
.
...
val
stringClass = parser.parseExpression(
"T(String)"
).getValue(Class::
class
.
...
Copied!
```
If your application or framework manages its own
EvaluationContext
, you may need to
