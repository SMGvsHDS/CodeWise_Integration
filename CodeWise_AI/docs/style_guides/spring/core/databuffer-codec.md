...
beneficial for performance. For example Netty has the
ByteBuf
...
more efficient even though implementations of
DataBuffer
...
or
java.nio.ByteBuffer
, which decorates the given data with
a
DataBuffer
implementation and that does not involve allocation.
Note that WebFlux applications do not create a
DataBufferFactory
...
The
DataBuffer
interface offers similar operations as
java.nio.ByteBuffer
but also
brings a few additional benefits some of which are inspired by the Netty
ByteBuf
.
...
is essential for byte buffer pooling. How does it work? When a
PooledDataBuffer
is
allocated the reference count is at 1. Calls to
retain()
...
the memory pool.
Note that instead of operating on
PooledDataBuffer
directly, in most cases it’s better
to use the convenience methods in
DataBufferUtils
that apply release or retain to a
DataBuffer
only if it is an instance of
PooledDataBuffer
.
DataBufferUtils
DataBufferUtils
...
DataBuffer
if the buffer is an instance of
PooledDataBuffer
.
...
since they may be
pooled
. We’ll use codecs to illustrate
...
If a
Decoder
simply reads each input buffer and is ready to
release it immediately, it can do so via
...
,
skip
, and others that leave out items, then
...
If a
Decoder
holds on to one or more data buffers in any other way, it must
...
buffer stream into a single data buffer. Likewise
skipUntilByteCount
and
takeUntilByteCount
...
An
Encoder
allocates data buffers that others must read (and release). So an
Encoder
doesn’t have much to do. However an
Encoder
...
In a WebFlux application, the output of the
Encoder
