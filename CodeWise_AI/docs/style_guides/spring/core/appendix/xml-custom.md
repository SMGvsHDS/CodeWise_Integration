...
Spring distribution, you should first read the previous section on
XML Schemas
.
...
we will be able to define bean definitions of type
SimpleDateFormat
as follows:
```
<
...
id
attribute that we can use as the bean identifier in the
container). We can use this attribute because we imported the Spring-provided
beans
namespace.
The preceding schema lets us configure
SimpleDateFormat
objects directly in an
XML application context file by using the
<myns:dateformat/>
...
creates a bean in the container (identified by the name
dateFormat
of type
SimpleDateFormat
...
Coding a
NamespaceHandler
In addition to the schema, we need a
NamespaceHandler
...
NamespaceHandler
should take care of the parsing of the
myns:dateformat
element.
The
NamespaceHandler
...
init()
: Allows for initialization of the
NamespaceHandler
and is called by
...
Although you can code your own
NamespaceHandler
for the entire
...
delegation. It supports the registration of any number of
BeanDefinitionParser
instances, to which it delegates to when it needs to parse an element in its
namespace. This clean separation of concerns lets a
NamespaceHandler
handle the
...
String pattern = element.getAttribute(
"pattern"
);
bean.addConstructorArgValue(pattern);
// this however is an optional property
String lenient = element.getAttribute(
"lenient"
);
if
(StringUtils.hasText(lenient)) {
...
AbstractSingleBeanDefinitionParser
to handle a lot of
the basic grunt work of creating a single
BeanDefinition
.
2
We supply the
AbstractSingleBeanDefinitionParser
superclass with the type that our
single
BeanDefinition
represents.
```
...
// this will never be null since the schema explicitly requires that a value be supplied
val
pattern = element.getAttribute(
"pattern"
)
bean.addConstructorArgValue(pattern)
// this however is an optional property
val
lenient = element.getAttribute(
"lenient"
)
if
(StringUtils.hasText(lenient)) {
...
properties files are both placed in a
META-INF
...
namespace extension and needs to exactly match exactly the value of the
targetNamespace
...
to prevent Spring from absolutely having to use a default
EntityResolver
that requires
...
that is actually configured by the
<foo:component/>
element is the
Component
class (shown in the next example). Notice how the
Component
class does not expose a
setter method for the
components
property. This makes it hard (or rather impossible)
to configure a bean definition for the
Component
...
Copied!
```
The typical solution to this issue is to create a custom
FactoryBean
that exposes a
...
(Element element)
{
BeanDefinitionBuilder factory = BeanDefinitionBuilder.rootBeanDefinition(ComponentFactoryBean
.
class
)
;
...
(Element element)
{
BeanDefinitionBuilder component = BeanDefinitionBuilder.rootBeanDefinition(Component
.
class
)
;
...
{
ManagedList<BeanDefinition> children =
new
ManagedList<>(childElements.size());
for
(Element element : childElements) {
...
: AbstractBeanDefinition {
val
factory = BeanDefinitionBuilder.rootBeanDefinition(ComponentFactoryBean::
class
.
java
)
...
: BeanDefinition {
val
component = BeanDefinitionBuilder.rootBeanDefinition(Component::
class
.
java
)
...
{
val
children = ManagedList<BeanDefinition>(childElements.size)
for
(element
in
childElements) {
...
JCache
, and you want to ensure that the
...
class
=
"com.foo.DefaultCheckingAccountService"
jcache:cache-name
=
"checking.account"
>
...
BeanDefinition
then initializes
the named JCache for us. We can also modify the existing
BeanDefinition
for the
'checkingAccountService'
...
JCache-initializing
BeanDefinition
. The following listing shows our
JCacheInitializer
:
Java
Kotlin
...
xmlns
=
"http://www.foo.example/schema/jcache"
xmlns:xsd
=
"http://www.w3.org/2001/XMLSchema"
targetNamespace
...
Copied!
```
Next, we need to create the associated
NamespaceHandler
, as follows:
Java
Kotlin
...
decorate
(Node source, BeanDefinitionHolder holder,
ParserContext ctx)
{
...
List dependencies =
new
ArrayList(Arrays.asList(dependsOn));
...
(Node source, ParserContext ctx)
{
String cacheName = ((Attr) source).getValue();
String beanName = cacheName +
"-initializer"
;
if
(!ctx.getRegistry().containsBeanDefinition(beanName)) {
BeanDefinitionBuilder initializer = BeanDefinitionBuilder.rootBeanDefinition(JCacheInitializer
.
class
)
;
...
: BeanDefinitionHolder {
val
initializerBeanName = registerJCacheInitializer(source, ctx)
createDependencyOnJCacheInitializer(holder, initializerBeanName)
return
holder
}
...
{
val
dependencies = ArrayList(listOf(*dependsOn))
dependencies.add(initializerBeanName)
...
-initializer"
if
(!ctx.registry.containsBeanDefinition(beanName)) {
val
initializer = BeanDefinitionBuilder.rootBeanDefinition(JCacheInitializer::
class
.
java
)
