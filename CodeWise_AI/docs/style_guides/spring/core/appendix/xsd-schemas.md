...
correct schema so that the tags in the
util
...
Copied!
```
The preceding configuration uses a Spring
FactoryBean
implementation (the
...
Copied!
```
There is also a convenience usage form where the
static
...
Copied!
```
This does mean that there is no longer any choice in what the bean
id
is (so any other
bean that refers to it also has to use this longer name), but this form is very
concise to define and very convenient to use as an inner bean since the
id
does not have
...
the Spring internals (or even about classes such as the
FieldRetrievingFactoryBean
).
...
Copied!
```
Now consider the following setter of type
PersistenceContextType
...
testBean
bean.
Now consider the following example, which adds a
<util:property-path/>
element:
```
<!-- target bean to be referenced by name -->
...
value
=
"classpath:com/foo/jdbc-production.properties"
/>
</
bean
>
...
Copied!
```
You can also explicitly control the exact type of
List
...
If no
list-class
attribute is supplied, the container chooses a
List
implementation.
Using
<util:map/>
...
snippet references the correct schema so that the tags in the
aop
namespace
...
a lot of the “grunt” work in Spring, such as
BeanfactoryPostProcessors
. The following
snippet references the correct schema so that the elements in the
context
namespace are
available to you:
```
...
specified properties file (as a
...
Spring’s
@EventListener
Alternatively, you can choose to explicitly activate the individual
BeanPostProcessors
...
.
The Beans Schema
Last but not least, we have the elements in the
beans
...
Note that you can add zero or more key-value pairs to
<bean/>
XML definitions.
...
in the appendix entitled
XML Schema Authoring
).
The following example shows the
<meta/>
element in the context of a surrounding
<bean/>
