...
and
@PreDestroy
annotations are generally considered best
...
no-argument signature. With Java configuration, you can use the
initMethod
attribute of
@Bean
. See
...
Be aware that
@PostConstruct
and initialization methods in general are executed
...
@PostConstruct
method. Such individual initialization methods are only meant
...
or rely on the context
refresh event: implementing
ApplicationListener<ContextRefreshedEvent>
or
declaring its annotation equivalent
...
Note that Spring also supports inference of destroy methods, detecting a public
close
or
shutdown
method. This is the default behavior for
@Bean
methods in Java configuration
classes and automatically matches
...
element a special
(inferred)
value, which instructs Spring to automatically
detect a public
close
or
shutdown
method on the bean class for a specific bean definition.
You can also set this special
(inferred)
value on the
...
of a
<beans>
element to apply this behavior to an entire set of bean definitions (see
Default Initialization and Destroy Methods
).
For extended shutdown phases, you may implement the
Lifecycle
...
SmartLifecycle
for a time-bound stop step where the container
...
can write your application classes and use an initialization callback called
init()
,
without having to configure an
init-method="init"
attribute with each bean definition.
The Spring IoC container calls that method when the bean is created (and in accordance
with the standard lifecycle callback contract
...
init()
for an initialization method — for more than one of these lifecycle mechanisms,
...
Copied!
```
Any Spring-managed object may implement the
Lifecycle
interface. Then, when the
ApplicationContext
itself receives start and stop signals (for example, for a stop/restart
scenario at runtime), it cascades those calls to all
Lifecycle
implementations
defined within that context. It does this by delegating to a
LifecycleProcessor
, shown
...
Notice that the
LifecycleProcessor
is itself an extension of the
Lifecycle
interface. It also adds two other methods for reacting to the context being refreshed
and closed.
Note that the regular
org.springframework.context.Lifecycle
interface is a plain
...
Lifecycle
beans first receive a stop notification before
...
prior to objects of another type. In those cases, the
SmartLifecycle
interface defines
another option, namely the
getPhase()
...
reverse order is followed. Therefore, an object that implements
SmartLifecycle
and
whose
getPhase()
...
The stop method defined by
SmartLifecycle
accepts a callback. Any
implementation must invoke that callback’s
run()
...
automatically for a standard context implementation). The
phase
value and any
...
ApplicationContext
implementations already have code in place to gracefully shut down
...
To register a shutdown hook, call the
...
providing visibility guarantees similar to
final
...
a
runnable
field which will have to be declared as
volatile
. While the common
...
Copied!
```
Thus, beans can programmatically manipulate the
ApplicationContext
that created them,
through the
ApplicationContext
...
parameter that expects the
ApplicationContext
type if the field, constructor, or
method in question carries the
@Autowired
...
dependency type. The following table summarizes the most important
Aware
interfaces:
