...
JSR 330’s
@Inject
annotation can be used in place of Spring’s
@Autowired
annotation in the
examples included in this section. See
here
for more details.
You can apply the
@Autowired
...
@Autowired
annotation on such a constructor is no longer
...
one of the constructors must be annotated with
@Autowired
...
are consistently declared by the type that you use for your
@Autowired
-annotated
...
@Resource
, which may obtain a proxy back to the current
...
lateinit
var
movieCatalogs: Array<MovieCatalog>
// ...
}
Copied!
```
...
lateinit
var
movieCatalogs: Set<MovieCatalog>
// ...
}
Copied!
```
...
order values do not affect the contained
@Bean
methods at all. For bean-level ordering,
each
@Bean
method needs to have its own
@Order
...
Even typed
Map
instances can be autowired as long as the expected key type is
String
.
...
{
private
Map<String, MovieCatalog> movieCatalogs;
@Autowired
public
void
setMovieCatalogs
...
lateinit
var
movieCatalogs: Map<String, MovieCatalog>
// ...
}
Copied!
```
...
non-required (i.e., by setting the
required
attribute in
@Autowired
to
...
optional
for autowiring purposes, and the property will be
...
, indicating
the
constructor to autowire when used as a Spring
bean. As a consequence, if the
required
attribute is left at its default value
true
,
...
declares multiple constructors but none of them is annotated with
@Autowired
, then a
...
void
setMovieFinder
(Optional<MovieFinder> movieFinder)
{
...
}
}
Copied!
```
You can also use a parameter-level
@Nullable
annotation (of any kind in any package — for example,
javax.annotation.Nullable
from JSR-305) or just leverage Kotlin built-in
null-safety support:
Java
Kotlin
```
...
void
setMovieFinder
(@Nullable MovieFinder movieFinder)
{
...
}
}
...
A type-level
@Nullable
annotation such as from JSpecify is not supported in Spring
...
types (if any).
These types must be 'wired up' explicitly by using XML or a Spring
@Bean
method.