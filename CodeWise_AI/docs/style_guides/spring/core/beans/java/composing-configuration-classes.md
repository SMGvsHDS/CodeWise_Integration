...
ConfigB.class
when
instantiating the context, only
ConfigB
...
new
AnnotationConfigApplicationContext(ConfigB
.
class
)
;
...
{
val
ctx = AnnotationConfigApplicationContext(ConfigB::
class
.
java
)
...
@Import
also supports references to regular component
classes, analogous to the
...
Injecting Dependencies on Imported
@Bean
Definitions
...
dependencies. Consider the following more realistic scenario with several
@Configuration
...
ApplicationContext ctx =
new
AnnotationConfigApplicationContext(SystemTestConfig
.
class
)
;
...
{
val
ctx = AnnotationConfigApplicationContext(SystemTestConfig::
class
.
java
)
...
Avoid access to locally defined beans within a
@PostConstruct
method on the same configuration
class. This effectively leads to a circular reference since non-static
@Bean
methods semantically
require a fully initialized configuration class instance to be called on. With circular references
disallowed (for example, in Spring Boot 2.6+), this may trigger a
BeanCurrentlyInCreationException
.
Also, be particularly careful with
BeanPostProcessor
and
...
through
@Bean
. Those should usually be declared as
static @Bean
methods, not triggering the
instantiation of their containing configuration class. Otherwise,
@Autowired
and
@Value
may not
...
@Configuration
classes is only supported as of Spring
Framework 4.3. Note also that there is no need to specify
@Autowired
if the target
...
still somewhat ambiguous. For example, as a developer looking at
ServiceConfig
, how do
...
provides tooling that
can render graphs showing how everything is wired, which may be all you need. Also,
your Java IDE can easily find all declarations and uses of the
AccountRepository
type
and quickly show you the location of
@Bean
methods that return that type.
In cases where this ambiguity is not acceptable and you wish to have direct navigation
from within your IDE from one
@Configuration
...
As of 6.2, there is a background initialization option:
...
of context startup. Only beans additionally marked as
@Lazy
are allowed to be completed
later (up until the first actual access).
Background initialization typically goes together with
@Lazy
(or
ObjectProvider
)
...
bean of type
Executor
must be declared for background
...
example of this is to use the
@Profile
annotation to activate beans only when a specific
profile has been enabled in the Spring
Environment
(see
...
for
(Object value : attrs.get(
"value"
)) {
if
(context.getEnvironment().matchesProfiles((String[]) value)) {
return
true
;
}
...
// Read the @Profile annotation attributes
val
attrs = metadata.getAllAnnotationAttributes(Profile::
class
.
java
.
...
if
(context.environment.matchesProfiles(*value
as
Array<String>)) {
return
true
...
Declaring
@Configuration
classes as plain Spring
<bean/>
elements
Remember that
@Configuration
classes are ultimately bean definitions in the container.
In this series of examples, we create a
@Configuration
class named
AppConfig
and
...
AppConfig
configuration class in Java and Kotlin:
Java
Kotlin
```
@Configuration
...
location
=
"classpath:/com/acme/jdbc.properties"
/>
<
bean
class
...
Similarly, the
DataSource
bean is only ever autowired by type, so an explicit bean
id
...
described in the previous example, we can redefine
system-test-config.xml
to take
...
enables the same
functionality.
The following example shows the modified
system-test-config.xml
file:
```
<
...
can use
@ImportResource
and define only as much XML as you need. Doing so achieves a
...
bean, a properties file, and the
main()
method) shows how to use the
@ImportResource
...
{
val
ctx = AnnotationConfigApplicationContext(AppConfig::
class
.
java
)
