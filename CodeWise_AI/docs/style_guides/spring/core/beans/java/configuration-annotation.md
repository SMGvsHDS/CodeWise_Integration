...
@Configuration
class. You cannot declare inter-bean dependencies
by using plain
@Component
classes.
...
package and included directly within the
spring-core
JAR.
There are a few restrictions due to the fact that CGLIB dynamically adds features at
startup-time. In particular, configuration classes must not be final. However, any
constructors are allowed on configuration classes, including the use of
@Autowired
or a
