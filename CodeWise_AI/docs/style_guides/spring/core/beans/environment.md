...
whether defined in XML or with annotations. The role of the
Environment
object with
...
variables, JNDI, servlet context parameters, ad-hoc
Properties
objects,
Map
objects, and so
...
with the production application server’s JNDI directory. Our
dataSource
bean
...
1
@Bean(destroyMethod = "")
disables default destroy method inference.
```
@Configuration
@Profile(
"production"
...
@Bean
methods, you typically choose to use programmatic
JNDI lookups, by using either Spring’s
JndiTemplate
/
JndiLocatorDelegate
helpers or the
straight JNDI
InitialContext
usage shown earlier but not the
JndiObjectFactoryBean
variant, which would force you to declare the return type as the
FactoryBean
type.
The profile string may contain a simple profile name (for example,
production
) or a
...
and
|
operators without using parentheses. For example,
production & us-east | eu-central
is not a valid expression. It must be expressed as
production & (us-east | eu-central)
.
You can use
@Profile
...
the specified profiles are active. If a
@Component
or
@Configuration
class is marked
with
@Profile({"p1", "p2"})
, that class is not registered or processed unless
profiles 'p1' or 'p2' have been activated. If a given profile is prefixed with the
NOT operator (
!
), the annotated element is registered only if the profile is not
active. For example, given
@Profile({"p1", "!p2"})
, registration will occur if profile
...
The
standaloneDataSource
method is available only in the
development
profile.
2
The
...
first declaration among the overloaded methods matters. Therefore,
@Profile
can
...
Copied!
```
It is also possible to avoid that split and nest
<beans/>
...
!
operator. It is also possible to apply a logical
...
variables, JVM system properties, servlet context parameters in
web.xml
, or even as an
entry in JNDI (see
PropertySource
Abstraction
). In integration tests, active
profiles can be declared by using the
@ActiveProfiles
annotation in the
spring-test
module (see
...
the default profile by using
setDefaultProfiles()
on the
Environment
or,
declaratively, by using the
...
boolean
containsMyProperty = env.containsProperty(
"my-property"
);
System.out.println(
"Does my environment contain the 'my-property' property? "
+ containsMyProperty);
Copied!
```
```
...
my-property
property is
defined for the current environment. To answer this question, the
Environment
object performs
a search over a set of
PropertySource
objects. A
PropertySource
...
my-property
property happens to be set in both places during
a call to
...
For a common
StandardServletEnvironment
, the full hierarchy is as follows, with the
...
and instantiate your own
PropertySource
and add it to the set of
PropertySources
for the
current
Environment
...
ctx = GenericApplicationContext()
val
sources = ctx.environment.propertySources
...
Given a file called
app.properties
that contains the key-value pair
...
resolved to the corresponding value. If not, then
default/path
is used
...
customer
property is defined, as long as it is available in the
Environment
:
```
<
