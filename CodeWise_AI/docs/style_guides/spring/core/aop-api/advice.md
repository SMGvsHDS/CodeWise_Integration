...
invoke()
method should return the invocation’s result: typically the return value of
the join point.
...
MethodInterceptor
implementations offer interoperability with other AOP Alliance-compliant AOP
...
in using the most specific advice type, stick with
MethodInterceptor
around advice if
...
Before Advice
A simpler advice type is a
before advice
. This does not need a
MethodInvocation
object, since it is called only before entering the method.
The main advantage of a before advice is that there is no need to invoke the
proceed()
...
Throws Advice
Throws advice
is invoked after the return of the join point if the join point threw
...
RemoteException
is thrown (including subclasses of
RemoteException
):
Java
Kotlin
...
coupled to specific target method signatures.
...
void
afterReturning
(Object returnValue, Method m, Object[] args, Object target)
throws
Throwable
;
}
...
Spring treats
introduction advice
as a special kind of interception advice.
Introduction requires an
IntroductionAdvisor
and an
...
The
invoke()
method inherited from the AOP Alliance
MethodInterceptor
interface must
...
Copied!
```
This illustrates a mixin. We want to be able to cast advised objects to
Lockable
,
whatever their type and call lock and unlock methods. If we call the
lock()
method, we
want all setter methods to throw a
LockedException
...
convenience class. We could implement
IntroductionInterceptor
directly, but using
...
default delegate (when the no-argument constructor is used) is
this
...
DelegatingIntroductionInterceptor
.
Given a delegate (by default, itself), a
DelegatingIntroductionInterceptor
instance
...
and implements
Lockable
itself. The superclass automatically picks up that
Lockable
...
{
if
(locked() && invocation.getMethod().getName().indexOf(
"set"
) ==
0
) {
...
: Any? {
if
(locked() && invocation.method.name.indexOf(
"set"
) ==
0
) {
...
Copied!
```
Often, you need not override the
invoke()
method. The
DelegatingIntroductionInterceptor
implementation (which calls the
delegate
method if
...
configuration relevant for a
LockMixin
, so we create it by using
new
.
The following example shows our
LockMixinAdvisor
class:
Java
Kotlin
