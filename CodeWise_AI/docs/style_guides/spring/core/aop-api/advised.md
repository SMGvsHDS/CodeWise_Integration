...
other advice type that has been added to the factory. If you added an
Advisor
, the
...
pointcut that always returns
true
. Thus, if you added a
MethodInterceptor
, the advisor
returned for this index is a
...
pointcut and advice is the generic
DefaultPointcutAdvisor
...
The following example shows casting an AOP proxy to the
Advised
...
int
oldAdvisorCount = advisors.length;
System.out.println(oldAdvisorCount +
" advisors"
);
...
advised.addAdvisor(
new
DefaultPointcutAdvisor(mySpecialPointcut, myAdvice));
assertEquals(
"Added two advisors"
, oldAdvisorCount +
2
...
Advised
val
advisors = advised.advisors
val
oldAdvisorCount = advisors.size
println(
"
$oldAdvisorCount
advisors"
...
advice through addition or removal results in an
AopConfigException
. The ability
