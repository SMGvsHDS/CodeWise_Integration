...
If you use the Spring IoC container (an
ApplicationContext
or
BeanFactory
) for your
...
level of indirection. If you define a
ProxyFactoryBean
named
foo
, objects that
...
Specify whether to use CGLIB (described later and see also
...
target class’s interfaces. If this property value is set to
true
, then CGLIB proxies
are created (but see also
JDK- and CGLIB-based proxies
).
optimize
...
when you do not want callers to be able to manipulate the proxy (through the
Advised
...
: An array of
String
interface names. If this is not supplied, a CGLIB
proxy for the target class is used (but see also
JDK- and CGLIB-based proxies
).
interceptorNames
: A
...
to be applied. You can find an example of using this feature in
Using “Global” Advisors
.
singleton: Whether or not the factory should return a single object, no matter how
often the
getObject()
method is called. Several
FactoryBean
implementations offer
such a method. The default value is
true
...
The behavior of the
ProxyFactoryBean
with regard to creating JDK- or CGLIB-based
proxies changed between versions 1.2.x and 2.0 of Spring. The
ProxyFactoryBean
now
...
and specify the list of interceptors by setting the
interceptorNames
property. Note that a
CGLIB-based proxy is created even if the
proxyTargetClass
property of the
ProxyFactoryBean
has been set to
...
created depends on the configuration of the
ProxyFactoryBean
.
If the
proxyTargetClass
...
principle of least surprise. Even if the
proxyInterfaces
property of the
ProxyFactoryBean
...
proxy implements all of the interfaces that were specified in the
proxyInterfaces
...
interface that the target class implements to the
proxyInterfaces
property. However,
...
that, if the singleton property of the
ProxyFactoryBean
is set to
false
, it must be able to
...
The
person
bean definition shown earlier can be used in place of a
Person
implementation, as
follows:
Java
Kotlin
```
Person person = (Person) factory.getBean(
"person"
);
Copied!
```
...
The
PersonUser
class in this example exposes a property of type
Person
. As far as
...
Copied!
```
Using an anonymous inner bean has the advantage that there is only one object of type
Person
...
arguably, an advantage in that the
ProxyFactoryBean
...
Imagine that in our earlier example, there was no
Person
...
in the
spring-core
JAR. In other words, CGLIB-based AOP works "out of the box", as do
JDK dynamic proxies.
