...
open-source class definition library (repackaged into
spring-core
).
...
case, you cannot create a CGLIB proxy for a class from the
java.lang
package when
...
Multiple
<aop:config/>
sections are collapsed into a single unified auto-proxy creator
at runtime, which applies the
strongest
proxy settings that any of the
<aop:config/>
...
, or
<aop:config/>
elements forces the use of CGLIB
proxies
...
{
val
factory = ProxyFactory(SimplePojo())
factory.addInterface(Pojo::
class
.
java
...
once the call has finally reached the target object (the
SimplePojo
reference in
this case), any method calls that it may make on itself, such as
this.bar()
or
this.foo()
...
self invocation via an explicit or implicit
this
reference will bypass the advice.
...
self injection
,
and invoke methods on the proxy via the self reference instead of via
this
.
Use
...
The use of
AopContext.currentProxy()
totally couples your code to Spring AOP, and it
makes the class itself aware of the fact that it is being used in an AOP context, which
reduces some of the benefits of AOP. It also requires that the
ProxyFactory
is
