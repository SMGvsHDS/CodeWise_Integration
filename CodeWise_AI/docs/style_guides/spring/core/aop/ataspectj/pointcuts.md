...
expression clear. The following example defines a pointcut named
anyOldTransfer
that
matches the execution of any method named
transfer
:
Java
Kotlin
...
) or one of the books on AspectJ (such as
Eclipse AspectJ
, by Colyer
et al., or
AspectJ in Action
...
within
: Limits matching to join points within certain types (the execution
of a method declared within a matching type when using Spring AOP).
this
: Limits matching to join points (the execution of methods when using Spring
AOP) where the bean reference (Spring AOP proxy) is an instance of the given type.
target
: Limits matching to join points (the execution of methods when using
...
using Spring AOP).
@annotation
: Limits matching to join points where the subject of the join point
...
expressions interpreted by Spring AOP results in an
...
class, consider the use of Spring-driven
native AspectJ weaving
instead
...
conventions for your Spring beans, you can write a
bean
PCD expression
to select them. As is the case with other pointcut designators, the
bean
PCD can
be used with the
&&
...
The
bean
PCD is supported only in Spring AOP and not in
native AspectJ weaving. It is a Spring-specific extension to the standard PCDs that
AspectJ defines and is, therefore, not available for aspects declared in the
@Aspect
model.
The
bean
PCD operates at the instance level (building on the Spring bean name
...
1
publicMethod
matches if a method execution join point represents the execution
...
named
pointcuts
, as shown above. When referring to pointcuts by name, normal Java visibility
rules apply (you can see
private
pointcuts in the same type,
protected
pointcuts in
the hierarchy,
public
...
We recommend defining a dedicated class that captures commonly used
named pointcut
...
```
this
is more commonly used in a binding form. See the section on
Declaring Advice
...
```
target
is more commonly used in a binding form. See the
Declaring Advice
section
...
and where the argument passed at runtime is
Serializable
:
```
...
Note that the pointcut given in this example is different from
execution(*
...
and where the runtime type of the argument passed has the
@Classified
annotation:
```
