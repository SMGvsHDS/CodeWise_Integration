...
class that is an @AspectJ aspect (has the
@Aspect
...
that points to a bean class that is annotated with
@Aspect
:
Java
Kotlin
...
class
=
"org.springframework.docs.core.aop.ataspectj.aopataspectj.NotVeryUsefulAspect"
>
<!-- configure properties of the aspect here -->
</
bean
...
Copied!
```
The second of the two examples shows the
NotVeryUsefulAspect
...
methods in
@Configuration
classes, or have Spring autodetect them through
...
purpose, you need to add a separate
@Component
annotation (or, alternatively, a custom
...
aspects. The
@Aspect
annotation on a class marks it as an aspect and, hence, excludes
