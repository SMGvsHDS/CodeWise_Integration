...
manifest entries which define stable language-level module names
(
spring.core
,
spring.context
, etc.) independent from jar artifact names. The jars follow
the same naming pattern with
-
instead of
.
– for example,
...
(
JSR 250
) specifications, which application
developers may choose to use instead of the Spring-specific mechanisms provided
by the Spring Framework. Originally, those were based on common
javax
packages.
As of Spring Framework 6.0, Spring has been upgraded to the Jakarta EE 9 level
(for example, Servlet 5.0+, JPA 3.0+), based on the
jakarta
namespace instead of the
traditional
javax
packages. With EE 9 as the minimum and EE 10 supported already,
...
issue tracker, and release cadence. See
spring.io/projects
for
...
You can use
start.spring.io
to generate a basic project or follow
one of the
