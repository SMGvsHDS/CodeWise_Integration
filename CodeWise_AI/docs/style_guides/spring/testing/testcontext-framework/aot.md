...
will be
refreshed for AOT processing
.
Runtime AOT support: when executing in AOT runtime mode, a Spring integration test will
use an AOT-optimized
ApplicationContext
that participates transparently with the
context cache
.
...
you can disable the
failOnError
mode which results in errors being logged at
WARN
level or with greater detail at
DEBUG
level.
The
failOnError
mode can be disabled from the command line or a build script by setting
...
See
Runtime Hints
for details on Spring’s core runtime hints
and annotation support.
The
TestRuntimeHintsRegistrar
API serves as a companion to the core
RuntimeHintsRegistrar
API. If you need to register global hints for testing support
that are not specific to particular test classes, favor implementing
RuntimeHintsRegistrar
over the test-specific API.
...
in order to participate in AOT processing. See the
