...
ApplicationContext
for a test class, by annotating the test class or one or more
...
The three annotations mentioned above build upon the
@BeanOverride
meta-annotation and
...
BeanOverrideHandler
implementations created by the processor
...
@BeanOverride
and instantiates the corresponding
BeanOverrideProcessor
which is
...
then uses bean override handlers to
alter the test’s
ApplicationContext
by creating, replacing, or wrapping beans as
...
FactoryBean
will be wrapped, not the
FactoryBean
itself.
In contrast to Spring’s autowiring mechanism (for example, resolution of an
@Autowired
field), the bean overriding infrastructure in the TestContext framework has limited
heuristics it can perform to locate a bean. Either the
BeanOverrideProcessor
can compute
