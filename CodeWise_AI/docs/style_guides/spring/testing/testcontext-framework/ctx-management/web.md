...
To instruct the TestContext framework to load a
WebApplicationContext
instead of a
standard
ApplicationContext
...
framework (TCF) that a
WebApplicationContext
(WAC) should be loaded for your
integration tests. In the background, the TCF makes sure that a
MockServletContext
is
...
override this default, you can provide an alternate path to the
@WebAppConfiguration
...
classpath:
prefix.
Note that Spring’s testing support for
WebApplicationContext
implementations is on par
with its support for standard
ApplicationContext
...
any other test annotations, such as
@ActiveProfiles
,
...
base path, the resource path effectively defaults to
...
configuration by using conventions (that is,
...
// classpath resource
@ContextConfiguration(
"/spring/test-servlet-config.xml"
)
class
WacTests
{
