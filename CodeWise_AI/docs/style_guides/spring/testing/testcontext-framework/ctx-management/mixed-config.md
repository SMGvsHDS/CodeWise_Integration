...
production, you may decide that you want to use
@Configuration
...
standard deployments. Consequently, most of the
SmartContextLoader
implementations that
the Spring Framework delivers in the
spring-test
module support only one resource type
for each test context. However, this does not mean that you cannot use both. One
exception to the general rule is that the
...
If you want to use resource locations (for example, XML or Groovy) and
@Configuration
...
@Configuration
classes from which your production
ApplicationContext
