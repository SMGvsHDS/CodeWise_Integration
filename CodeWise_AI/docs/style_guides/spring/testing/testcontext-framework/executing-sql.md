...
initializing
an embedded or existing database
by executing SQL scripts when the Spring
ApplicationContext
is loaded. See
...
for details.
Although it is very useful to initialize a database for testing
once
when the
ApplicationContext
...
database
during
integration tests. The following sections explain how to run SQL
...
require full control over how SQL scripts are parsed and run,
ScriptUtils
may suit
...
ResourceDatabasePopulator
, you can invoke either the
populate(Connection)
method to
run the populator against a
java.sql.Connection
or the
execute(DataSource)
method
to run the populator against a
javax.sql.DataSource
. The following example
specifies SQL scripts for a test schema and test data, sets the statement separator to
@@
, and run the scripts against a
DataSource
:
Java
Kotlin
...
ScriptUtils
for parsing
and running SQL scripts. Similarly, the
executeSqlScript(..)
methods in
...
Specifically, you can declare the
@Sql
...
Method-level
@Sql
declarations override class-level declarations by default, but this
behavior may be configured per test class or per test method via
@SqlMergeMode
. See
Merging and Overriding Configuration with
@SqlMergeMode
for further details.
...
or
AFTER_TEST_CLASS
execution phases. Such declarations cannot be
...
Each path is interpreted as a Spring
Resource
...
be replaced by properties stored in the
Environment
of the test’s
ApplicationContext
.
...
If no SQL scripts or statements are specified, an attempt is made to detect a
default
script, depending on where
@Sql
...
different execution phases per set, you can declare multiple instances of
@Sql
. You can
either use
@Sql
as a repeatable annotation, or you can use the
@SqlGroup
annotation
as an explicit container for declaring multiple instances of
@Sql
.
The following example shows how to use
@Sql
...
Copied!
```
In the scenario presented in the preceding example, the
test-schema.sql
script uses a
different syntax for single-line comments.
The following example is identical to the preceding example, except that the
@Sql
declarations are grouped together within
@SqlGroup
. The use of
@SqlGroup
is optional,
...
database state), you can set the
executionPhase
attribute in
@Sql
to
...
after the test class by setting the
executionPhase
attribute in a class-level
@Sql
declaration to
BEFORE_TEST_CLASS
or
...
```
BEFORE_TEST_CLASS
is statically imported from
Sql.ExecutionPhase
.
Script Configuration with
@SqlConfig
You can configure script parsing and error handling by using the
@SqlConfig
annotation.
...
declared directly by using the
config
attribute of the
@Sql
annotation,
@SqlConfig
serves as local configuration for the SQL scripts declared within the enclosing
@Sql
annotation. Every attribute in
@SqlConfig
...
possible to assign a value of
null
to an annotation attribute. Thus, in order to
support overrides of inherited global configuration,
@SqlConfig
attributes have an
...
DEFAULT
(for
enumerations). This approach lets local declarations of
@SqlConfig
selectively override
individual attributes from global declarations of
@SqlConfig
...
semantics for scripts configured by using
@Sql
...
you can specify explicit names by setting the
dataSource
and
transactionManager
attributes of
...
supported options for transaction management with
@Sql
...
that uses JUnit Jupiter and transactional tests with
@Sql
:
Java
Kotlin
...
expected)
{
assertEquals(expected, countRowsInTable(
"user"
),
...
Copied!
```
Note that there is no need to clean up the database after the
usersTest()
method is
...
Merging and Overriding Configuration with
@SqlMergeMode
It is possible to merge method-level
@Sql
declarations with
...
use case specific test data per test method. To enable
@Sql
