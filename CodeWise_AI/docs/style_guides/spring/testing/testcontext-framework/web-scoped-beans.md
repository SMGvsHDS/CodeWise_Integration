...
userService
bean has a dependency on a request-scoped
loginAction
bean. Also, the
LoginAction
is instantiated by using
SpEL expressions
that
...
userService
, we are assured that
the user service has access to the session-scoped
userPreferences
for the current
MockHttpSession
...
val
results = userService.processUserPreferences()
// assert results
}
}
Copied!
