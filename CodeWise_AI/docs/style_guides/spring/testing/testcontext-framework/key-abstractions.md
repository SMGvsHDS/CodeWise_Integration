...
all test methods within a single test class in JUnit Jupiter). The
TestContextManager
,
in turn, manages a
TestContext
...
the test instance for which it is responsible. The
TestContext
also delegates to a
SmartContextLoader
to load an
...
Context Loaders
ContextLoader
is a strategy interface for loading an
ApplicationContext
for an
