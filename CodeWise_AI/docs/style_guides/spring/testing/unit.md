...
should be testable in JUnit or TestNG tests, with objects instantiated by using the
new
...
convenient to use than dynamic mock objects (such as
EasyMock
).
...
example, a mock request is immutable once created, but you can use the
mutate()
method
from
ServerHttpRequest
...
write completion handle (that is,
Mono<Void>
), it by default uses a
Flux
with
cache().then()
...
The
WebTestClient
builds on the mock request and response to provide support for
...
scenarios where you need to change the value of a constant, set a non-
public
field,
invoke a non-
public
setter method, or invoke a non-
public
...
that provide dependency injection for
private
or
protected
...
TestSocketUtils
is a simple
utility for finding available TCP ports on
localhost
for use in integration testing
scenarios.
TestSocketUtils
can be used in integration tests which start an external server on an
...
TestSocketUtils
to find an available local port for a server, it is recommended that
...
that deal with Spring MVC
ModelAndView
objects.
Unit testing Spring MVC Controllers
To unit test your Spring MVC
Controller
classes as POJOs, use
ModelAndViewAssert
combined with
MockHttpServletRequest
,
MockHttpSession
, and so on from Spring’s
Servlet API mocks
. For thorough integration
testing of your Spring MVC and REST
Controller
classes in conjunction with your
WebApplicationContext
configuration for Spring MVC, use
MockMvc
instead.