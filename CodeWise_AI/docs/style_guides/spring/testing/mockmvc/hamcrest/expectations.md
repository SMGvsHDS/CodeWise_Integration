...
You can define expectations by appending one or more
andExpect(..)
calls after
...
Copied!
```
You can define multiple expectations by appending
andExpectAll(..)
after performing a
request, as the following example shows. In contrast to
andExpect(..)
,
andExpectAll(..)
...
request. You can do so as follows, where
print()
...
print()
method
prints all the available result data to
System.out
. There is also a
log()
method and
two additional variants of the
print()
...
cannot be verified otherwise. This can be achieved by appending
.andReturn()
after all
