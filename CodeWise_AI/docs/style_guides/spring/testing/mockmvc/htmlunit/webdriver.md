...
Kotlin
```
HtmlTextInput summaryInput = currentPage.getHtmlElementById(
"summary"
);
...
HtmlPage
createMessage
(HtmlPage currentPage, String summary, String text)
{
...
void
setSummary
(HtmlPage currentPage, String summary)
{
HtmlTextInput summaryInput = currentPage.getHtmlElementById(
"summary"
);
...
this
.submit = currentPage.getHtmlElementById(
"submit"
);
}
public
...
boolean
error = CreateMessagePage.at(result);
return
(T) (error ?
new
CreateMessagePage(result) :
new
...
val
summaryInput: HtmlTextInput = currentPage.getHtmlElementById(
"summary"
)
val
submit: HtmlSubmitInput = currentPage.getHtmlElementById(
"submit"
)
fun
<T>
...
(
if
(error) CreateMessagePage(result)
else
ViewMessagePage(result))
as
T
...
To use Selenium WebDriver with
MockMvc
...
MockMvcHtmlUnitDriverBuilder
.
The preceding example ensures that any URL that references
localhost
as the server is
directed to our
MockMvc
...
```
val
page = CreateMessagePage.to(driver)
Copied!
```
...
page.createMessage(ViewMessagePage
.
class
,
expectedSummary
...
Why WebDriver and MockMvc?
, we can use the Page Object Pattern
...
T
createMessage
(Class<T> resultPage, String summary, String details)
{
this
...
{
driver.get(
"http://localhost:9990/mail/messages/form"
);
return
PageFactory.initElements(driver, CreateMessagePage
.
class
)
;
...
AbstractPage
, but, in summary, it contains common functionality for all of our pages.
...
interested. These are of type
WebElement
. WebDriver’s
PageFactory
lets us remove a
lot of code from the HtmlUnit version of
CreateMessagePage
...
method automatically resolves each
WebElement
by using the field name and looking it up
by the
id
or
name
...
@FindBy
annotation
to override the default lookup behavior. Our example shows how to use the
@FindBy
annotation to look up our submit button with a
css
selector (
input[type=submit]
).
...
assertThat(viewMessagePage.getSuccess()).isEqualTo(
...
assertThat(viewMessagePage.success).isEqualTo(
"Successfully created a new message"
)
Copied!
```
...
example, it exposes a method that returns a
Message
object:
Java
Kotlin
...
getMessage
()
= Message(getId(), getCreated(), getSummary(), getText())
Copied!
```
We can then use the rich domain objects in our assertions.
Lastly, we must not forget to close the
WebDriver
