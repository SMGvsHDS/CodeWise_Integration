...
TaskExecutor
Types
Spring includes a number of pre-built implementations of
TaskExecutor
.
...
are looking for true pooling, see
...
:
This implementation is an adapter for a
java.util.concurrent.Executor
instance.
...
TaskExecutor
.
If you need to adapt to a different kind of
java.util.concurrent.Executor
,
...
:
This implementation uses a JNDI-obtained
...
In the following example, we define a bean that uses the
ThreadPoolTaskExecutor
...
return
new
TaskExecutorExample(taskExecutor);
}
Copied!
```
```
...
Most
TaskExecutor
implementations provide a way to automatically wrap tasks submitted
with a
TaskDecorator
. Decorators should delegate to the task it is wrapping, possibly
...
static
final
Log logger = LogFactory.getLog(LoggingTaskDecorator
.
class
)
;
...
Copied!
```
We can then configure our decorator on a
TaskExecutor
instance:
Java
Kotlin
...
Copied!
```
In case multiple decorators are needed, the
...
Copied!
```
The simplest method is the one named
schedule
that takes only a
Runnable
and an
...
methods are for simple, periodic execution, but the method that accepts a
Trigger
is
much more flexible.
Trigger
...
preceding execution, that information is available within a
TriggerContext
. The
Trigger
...
default). The following listing shows the available methods for
Trigger
implementations.
```
public
...
application itself. For such scenarios, Spring provides a
DefaultManagedTaskScheduler
that delegates to a JSR-236
...
, which internally delegates to a
ScheduledExecutorService
to provide common bean-style configuration along the lines of
ThreadPoolTaskExecutor
.
...
if you need only support for
@Scheduled
, you can omit
@EnableAsync
. For more
fine-grained control, you can additionally implement the
...
tasks that correspond to methods with the
@Async
annotation, and the scheduler
reference is provided for managing those methods annotated with
@Scheduled
.
...
minutes, you can configure this via the
timeUnit
attribute in
@Scheduled
.
...
Copied!
```
If you need a fixed-rate execution, you can use the
fixedRate
...
instance. Related to this, make sure that you do not use
@Configurable
on bean
...
methods with a
Publisher
return type (or any concrete implementation of
Publisher
)
...
to
Publisher
but doesn’t support deferred subscription. Its
ReactiveAdapter
in the
registry denotes that by having the
getDescriptor().isDeferred()
method return
false
.
...
the framework to invoke a suspending function as a
Publisher
.
The Spring Framework will obtain a
Publisher
for the annotated method once and will
schedule a
Runnable
in which it subscribes to said
Publisher
. These inner regular
subscriptions occur according to the corresponding
cron
/
fixedDelay
/
...
return values from synchronous
@Scheduled
methods are ignored).
...
emits an
onError
signal, it is logged at
WARN
level and recovered.
Because of the asynchronous and lazy nature of
Publisher
instances, exceptions are
not thrown from the
Runnable
task: this means that the
ErrorHandler
contract is not
...
code is a legitimate application of the
@Async
annotation:
```
@Async
...
```
@Async
methods may not only declare a regular
java.util.concurrent.Future
return
type but also Spring’s
...
initializing Spring bean that then invokes the
@Async
...
However, you can manually set up Spring’s
AsyncExecutionInterceptor
with Spring AOP,
in combination with a custom pointcut.
Executor Qualification with
@Async
By default, when specifying
@Async
on a method, the executor that is used is the
one
configured when enabling async support
,
i.e. the “annotation-driven” element if you are using XML or your
AsyncConfigurer
implementation, if any. However, you can use the
value
attribute of the
@Async
...
Executor
bean in the Spring
container, or it may be the name of a qualifier associated with any
Executor
(for example,
...
Copied!
```
By default, the exception is merely logged. You can define a custom
AsyncUncaughtExceptionHandler
by using
AsyncConfigurer
or the
...
task
Namespace
As of version 3.0, Spring includes an XML namespace for configuring
TaskExecutor
and
TaskScheduler
...
scheduler
Element
The following element creates a
ThreadPoolTaskScheduler
instance with the
...
Copied!
```
As with the scheduler shown in the
previous section
,
the value provided for the
id
attribute is used as the prefix for thread names within
the pool. As far as the pool size is concerned, the
executor
...
max sizes are the same). However, the
executor
element’s
pool-size
attribute also
accepts a range in the form of
min-max
...
Copied!
```
In the preceding configuration, a
queue-capacity
...
task is rejected, a thread pool executor throws a
TaskRejectedException
. However,
the rejection policy is actually configurable. The exception is thrown when using
the default rejection policy, which is the
AbortPolicy
implementation.
...
for configuring message-driven POJOs. Basically, a
ref
attribute can point to any
Spring-managed object, and the
method
...
any previous execution takes. Additionally, for both
fixed-delay
and
fixed-rate
...
.
If followed by a negative offset (that is,
L-n
), it means
n
...
.
If prefixed by a number or three-letter name (
dL
or
DDDL
), it means
...
nW
, which stands for
the nearest weekday to day of the month
n
.
If
n
...
If
n
falls on Sunday, this yields the Monday after, which also happens if
n
is
1
and falls on
...
Quartz Web site
. For convenience purposes, Spring
...
Copied!
```
The job detail configuration has all the information it needs to run the job (
ExampleJob
).
The timeout is specified in the job data map. The job data map is available through the
JobExecutionContext
(passed to you at execution time), but the
JobDetail
also gets
its properties from the job data mapped to properties of the job instance. So, in the
following example, the
ExampleJob
contains a bean property named
timeout
, and the
JobDetail
...
Copied!
```
The preceding example results in the
doIt
...
class
=
"examples.ExampleBusinessObject"
/>
Copied!
```
By using the
...
with each other. If you specify two triggers for the same
JobDetail
, it is possible
that the second one starts before the first job has finished. If
JobDetail
classes
implement the
Stateful
...
Wiring up Jobs by Using Triggers and
...
jobs themselves. This is done by using triggers and a
SchedulerFactoryBean
. Several
triggers are available within Quartz, and Spring offers two Quartz
FactoryBean
implementations with convenient defaults:
...
SchedulerFactoryBean
that exposes
triggers to be set as properties.
SchedulerFactoryBean
schedules the actual jobs with
those triggers.
The following listing uses both a
SimpleTriggerFactoryBean
and a
CronTriggerFactoryBean
:
...
variant which
is a full-fledged replacement for the standard
