...
and
<jca-listener-container/>
define shared listener container configuration and can contain
<listener/>
child elements.
...
attribute. Note that this applies only to a listener method with a
...
5
) or a range indicating the lower
as well as the upper limit (for example,
3-5
). Note that a specified minimum is only a hint
...
allows for customization of the various strategies (for example,
taskExecutor
and
destinationResolver
...
You can automatically expose such settings as a
JmsListenerContainerFactory
by
specifying the
id
of the bean to expose through the
factory-id
attribute,
...
A reference to the
DestinationResolver
strategy for resolving JMS
Destination
instances.
message-converter
A reference to the
MessageConverter
strategy for converting JMS Messages to listener
method arguments. The default is a
SimpleMessageConverter
.
error-handler
A reference to an
ErrorHandler
strategy for handling any uncaught exceptions that
may occur during the execution of the
MessageListener
.
destination-type
...
or
sharedDurableTopic
. This potentially enables the
pubSubDomain
,
subscriptionDurable
and
subscriptionShared
properties of the container. The default is
queue
(which disables
...
. By default (
auto
), the cache level is effectively
consumer
, unless
an external transaction manager has been specified — in which case, the effective
default will be
none
(assuming Jakarta EE-style transaction management, where the given
...
of
transacted
activates a locally transacted
Session
. As an alternative, you can specify
the
transaction-manager
attribute, described later in table. The default is
auto
.
transaction-manager
A reference to an external
PlatformTransactionManager
(typically an XA-based
transaction coordinator, such as Spring’s
JtaTransactionManager
). If not specified,
native acknowledging is used (see the
acknowledge
attribute).
concurrency
...
a simple number indicating the maximum number (for example,
5
) or a range indicating the
lower as well as the upper limit (for example,
3-5
). Note that a specified minimum is just a
hint and might be ignored at runtime. The default is
1
. You should keep concurrency limited to
1
in
...
Specifies the
BackOff
instance to use to compute the interval between recovery
attempts. If the
BackOffExecution
implementation returns
BackOffExecution#STOP
,
the listener container does not further try to recover. The
recovery-interval
value is ignored when this property is set. The default is a
FixedBackOff
with
...
way to create a
FixedBackOff
with the specified interval. For more recovery
options, consider specifying a
BackOff
instance instead. The default is 5000 milliseconds
...
Integer.MAX_VALUE
, meaning that the container starts as late as possible and stops as
soon as possible.
Configuring a JCA-based listener container with the
jms
...
. The default is to autodetect the JMS
provider and its
ActivationSpec
class (see
...
MessageConverter
strategy for converting JMS Messages to listener
method arguments. The default is
SimpleMessageConverter
.
destination-type
The JMS destination type for this listener:
...
the
transaction-manager
attribute described later. The default is
auto
.
transaction-manager
...
for kicking off an XA transaction for each
...
lower as well as the upper limit (for example,
3-5
). Note that a specified minimum is only a
