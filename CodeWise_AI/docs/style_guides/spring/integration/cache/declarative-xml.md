...
class
=
"x.y.service.DefaultBookService"
/>
<!-- cache definitions -->
<
cache:advice
...
to apply are encapsulated in the
cache:advice
definition, which causes the
findBooks
method to be used for putting data into the cache and the
loadBooks
method for evicting
data. Both definitions work against the
books
cache.
The
aop:config
...
caching, identifying the proper methods does take an extra effort, since the
method
...
in the preceding example by defining the target cache through the
cache:definitions
cache
attribute).