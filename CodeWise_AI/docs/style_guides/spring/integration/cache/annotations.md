...
@Cacheable
Annotation
As the name implies, you can use
@Cacheable
...
findBook
method is associated with the cache named
books
.
...
considered only the
hashCode()
of parameters and not
equals()
. This could cause
unexpected key collisions (see
spring-framework#14870
for background). The new
SimpleKeyGenerator
uses a compound key for such scenarios.
...
to be shared, you can define a custom
keyGenerator
...
and
keyGenerator
parameters are mutually exclusive and an operation
that specifies both results in an exception.
Default Cache Resolution
The caching abstraction uses a simple
CacheResolver
that retrieves the caches
defined at the operation level by using the configured
CacheManager
.
...
CacheResolver
.
Since Spring 4.1, the
value
attribute of the cache annotations are no longer
mandatory, since this particular information can be provided by the
CacheResolver
...
results in an exception, as a custom
CacheManager
is ignored by the
CacheResolver
...
of caching.
For those particular cases, you can use the
sync
...
is updated in the cache. The following example shows how to use the
sync
attribute:
```
@Cacheable
...
public
CompletableFuture<Book>
findBook
(ISBN isbn)
{...}
Copied!
```
For a method returning a Reactor
Mono
...
publisher will be collected into a
List
and cached whenever that list is complete,
and the cache lookup for a cache hit will be retrieved as a
Flux
(backed by a
CompletableFuture
for the cached
...
CompletableFuture
-based retrieval. The Spring-provided
ConcurrentMapCacheManager
automatically adapts to that retrieval style, and
CaffeineCacheManager
natively supports it when its asynchronous cache mode is
enabled: set
...
cacheManager.setAsyncCacheMode(
true
);
return
cacheManager;
...
emitted object for a
Mono
or even a pre-collected list of objects for a
Flux
.
Conditional Caching
...
adding of a value to the cache. Unlike
condition
,
unless
...
is
present
, it will be stored in the associated cache. If an
Optional
value is not
present,
null
...
Each
SpEL
expression evaluates against a dedicated
context
.
...
is
false
). For supported wrappers (such as
Optional
),
#result
...
result is placed into the cache (according to the
@CachePut
options). It supports
the same options as
@Cacheable
and should be used for cache population rather than
method flow optimization. The following example uses the
@CachePut
annotation:
```
@CachePut
...
Book
updateBook
(ISBN isbn, BookDescriptor descriptor)
Copied!
```
Using
@CachePut
...
on the result object (that is, the
#result
variable), as these are validated up-front to
...
Using the
allEntries
attribute to evict all entries from the cache.
...
the method is invoked by using the
beforeInvocation
...
@Caching
Annotation
Sometimes, multiple annotations of the same type (such as
@CacheEvict
or
@CachePut
) need to be specified — for example, because the condition or the key
expression is different between different caches.
@Caching
...
class can be replaced by a single class-level definition. This is where
@CacheConfig
comes into play. The following examples uses
@CacheConfig
...
An operation-level customization always overrides a customization set on
@CacheConfig
.
Therefore, this gives three levels of customizations for each cache operation:
Globally configured, for example, through
CachingConfigurer
: see next section.
...
Provider-specific settings are typically available on the
CacheManager
bean,
for example, on
CaffeineCacheManager
. These are effectively also global.
...
your code).
To enable caching annotations add the annotation
@EnableCaching
to one of your
@Configuration
classes or use the
...
@Transactional
.
The default advice mode for processing caching annotations is
proxy
, which allows
...
The default mode (
proxy
) processes annotated beans to be proxied by using Spring’s AOP
...
code to apply to any kind of method call. AspectJ weaving requires
spring-aspects.jar
...
attribute is set to
true
, class-based proxies are created.
If
proxy-target-class
is
false
or if the attribute is omitted, standard JDK
...
or
@CacheEvict
. (For more information about the rules related to
ordering AOP advice, see
Advice Ordering
.)
...
for a
DispatcherServlet
, it checks for beans only in your controllers, not your services.
See
the MVC section
...
classes) with the
@Cache*
annotations, as opposed to annotating interfaces.
You certainly can place an
@Cache*
annotation on an interface (or an interface
method), but this works only if you use the proxy mode (
mode="proxy"
). If you use the
weaving-based aspect (
mode="aspectj"
), the caching settings are not recognized on
...
using the
aspectj
mode in this case. Also, the proxy must be fully initialized to
provide the expected behavior, so you should not rely on this feature in your
initialization code (that is,
@PostConstruct
).
...
The
spring-aspects
module defines an aspect for the standard annotations only.
...
Copied!
```
In the preceding example, we have defined our own
SlowService
annotation,
