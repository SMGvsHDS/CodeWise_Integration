...
MBean, you can configure an instance of the
MBeanExporter
class in your
...
Copied!
```
The pertinent bean definition from the preceding configuration snippet is the
exporter
bean. The
beans
property tells the
...
corresponding entry value. You can change this behavior, as described in
Controlling
ObjectName
...
are exposed as attributes and all
public
...
the application lifecycle. You can configure the
phase
at which
...
preceding section
assumes that the
application is running in an environment that has one (and only one)
MBeanServer
already running. In this case, Spring tries to locate the running
MBeanServer
and
...
a container that does not provide an
MBeanServer
...
You can also ensure that a specific
MBeanServer
...
Copied!
```
In the preceding example, an instance of
MBeanServer
is created by the
MBeanServerFactoryBean
and is
...
MBeanServer
. This works in most environments, where only one
MBeanServer
instance is
...
Copied!
```
For platforms or cases where the existing
MBeanServer
...
class
=
"platform.package.MBeanServerLocator"
factory-method
=
"locateMBeanServer"
/>
...
If you configure a bean with an
MBeanExporter
that is also configured for lazy
initialization, the
MBeanExporter
does not break this contract and avoids
instantiating the bean. Instead, it registers a proxy with the
MBeanServer
and defers
...
MBeanExporter
will regularly
introspect the produced object, effectively triggering
...
MBeanExporter
and are already valid MBeans
are registered as-is with the
MBeanServer
without further intervention from Spring.
You can cause MBeans to be automatically detected by the
MBeanExporter
by setting
the
autodetect
...
. If an
MBean
instance has already been registered under that same
ObjectName
, the default behavior
is to fail (and throw an
InstanceAlreadyExistsException
).
You can control exactly what happens when an
MBean
is
registered with an
MBeanServer
...
, the
MBean
that is being registered is not
registered, and an
InstanceAlreadyExistsException
is thrown. The existing
MBean
is unaffected.
IGNORE_EXISTING
If an
MBean
instance has already been registered under the same
ObjectName
, the
MBean
that is being registered is not registered. The existing
MBean
is
unaffected, and no
Exception
is thrown. This is useful in settings where
multiple applications want to share a common
MBean
in a shared
MBeanServer
.
...
existing
MBean
that was previously registered is unregistered, and the new
MBean
is registered in its place (the new
MBean
effectively replaces the
previous instance).
The values in the preceding table are defined as enums on the
RegistrationPolicy
class.
