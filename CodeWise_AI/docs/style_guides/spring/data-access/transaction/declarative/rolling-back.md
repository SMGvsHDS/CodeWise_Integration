...
that a transaction’s work is to be rolled back is to throw an
Exception
from code that
is currently executing in the context of a transaction. The Spring Framework’s
transaction infrastructure code catches any unhandled
Exception
as it bubbles up
...
The default configuration also provides support for Vavr’s
Try
method to trigger
...
return
Try.of(delegate::myDataAccessOperation);
}
Copied!
```
As of Spring Framework 6.1, there is also special treatment of
CompletableFuture
(and general
Future
...
need to comply with a
CompletableFuture
signature (auto-adapted to an actual
asynchronous handle for a call to the proxy by
@Async
...
{
return
CompletableFuture.completedFuture(delegate.myDataAccessOperation());
}
catch
...
in the default configuration. You can configure exactly which
Exception
types mark a
transaction for rollback, including checked exceptions by specifying
rollback rules
.
Rollback rules
...
Rollback rules may be configured in XML via the
rollback-for
and
no-rollback-for
...
@Transactional
,
rollback rules may be configured via the
rollbackFor
/
noRollbackFor
and
...
attributes, which allow rules to be
...
will only match thrown exceptions of
type
...
Throwable
), with no wildcard support at present. For
example, a value of
"jakarta.servlet.ServletException"
or
"ServletException"
...
information (which isn’t mandatory). For example,
"Exception"
will match nearly
anything and will probably hide other rules.
"java.lang.Exception"
would be correct if
"Exception"
...
For example, given a rule configured to match on
"com.example.CustomException"
, that
rule will match against an exception named
