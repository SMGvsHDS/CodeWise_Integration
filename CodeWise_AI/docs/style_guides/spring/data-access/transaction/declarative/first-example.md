...
UnsupportedOperationException
instance. The following listing shows the
FooService
interface:
Java
Kotlin
...
TransactionManager
that you want to
wire in has the name
transactionManager
. If the
TransactionManager
bean that
you want to wire in has any other name, you must use the
transaction-manager
...
(
fooServiceOperation
). Then you associate the pointcut with the
txAdvice
by using an
advisor. The result indicates that, at the execution of a
fooServiceOperation
,
...
FooService fooService = ctx.getBean(FooService
.
class
)
;
...
output and the stack trace from the
UnsupportedOperationException
thrown by the
insertFoo(..)
method of the
...
to determine whether a method
return type is reactive.
The following listing shows a modified version of the previously used
FooService
, but
...
and reactive transactions is the deferred nature of the latter.
