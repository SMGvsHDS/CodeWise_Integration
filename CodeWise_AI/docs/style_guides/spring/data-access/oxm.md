...
As stated in the
introduction
, a marshaller serializes an object
...
Although the
marshal()
method accepts a plain object as its first parameter, most
Marshaller
...
, and
loadSettings()
loads these settings again. The following
main()
method
...
Application application = (Application) appContext.getBean(
"application"
);
...
()
{
FileInputStream(FILE_NAME).use { inputStream -> settings = unmarshaller.unmarshal(StreamSource(inputStream))
as
Settings }
}
}
...
val
application = appContext.getBean(
"application"
)
as
Application
...
The
Jaxb2Marshaller
class implements both of Spring’s
Marshaller
and
Unmarshaller
...
names that contain schema derived classes. It also offers a
classesToBeBound
property,
...
value
>
org.springframework.oxm.jaxb.Flight
</
value
>
<
value
>
org.springframework.oxm.jaxb.Flights
</
value
>
</
...
value
=
"classpath:org/springframework/oxm/schema.xsd"
/>
</
bean
>
...
from or to XML.
For more information on JiBX, see the
JiBX web
site
...
bindingName
property. In the following example, we bind the
Flights
class:
```
<
...
For more information on XStream, see the
XStream
web site
. The Spring integration classes reside in the
