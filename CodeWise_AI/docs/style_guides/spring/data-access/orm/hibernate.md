...
as well as for a native Hibernate
SessionFactory
setup.
We recommend Hibernate ORM 5.6 as the last feature branch in that Hibernate generation.
Hibernate ORM 6.x is primarily supported as a JPA provider (
HibernateJpaVendorAdapter
).
Plain
SessionFactory
setup with the
orm.hibernate5
package is tolerated for migration
...
variant, seamlessly integrating
with
@Bean
style configuration and programmatic setup (no
FactoryBean
involved).
Both
...
bootstrap thread on a given bootstrap executor (such as a
...
Such a native Hibernate setup can also expose a JPA
EntityManagerFactory
for standard
JPA interaction next to native Hibernate access.
See
Native Hibernate Setup for JPA
for details.
...
synchronization of one Hibernate
Session
...
private
val
sessionFactory: SessionFactory) : ProductDao {
fun
loadProductsByCategory
(category:
String
...
such an instance-based setup over the old-school
static
HibernateUtil
class from
...
Before you continue, we are strongly encourage you to read
...
public
void
increasePriceOfAllProductsInCategory
(
final
String category)
{
...
private
val
productDao: ProductDao) : ProductService {
@Transactional
fun
increasePriceOfAllProductsInCategory
(category:
...
{
val
productsToChange = productDao.loadProductsByCategory(category)
// ...
}
...
LocalSessionFactoryBean
definitions. Each DAO then gets one specific
SessionFactory
...
JTA, provided you access only one database.
HibernateTransactionManager
automatically
...
SessionFactory
, a manually registered JNDI
SessionFactory
does not provide any
benefits. Deploying a
SessionFactory
...
Pass your Spring
JtaTransactionManager
bean to your Hibernate setup. The easiest
way is a bean reference into the
jtaTransactionManager
property for your
LocalSessionFactoryBean
bean (see
...
without Hibernate’s awareness of the JTA
PlatformTransactionManager
.
...
transaction is called back through a
beforeCompletion
callback by the JTA
transaction manager.
...
through an
afterCompletion
callback by the JTA transaction manager and can
