...
setDataSource
method. You do not need to subclass the
SimpleJdbcInsert
class. Instead,
...
class
JdbcActorDao
(dataSource: DataSource) : ActorDao {
private
val
insertActor = SimpleJdbcInsert(dataSource).withTableName(
"t_actor"
)
fun
add
...
The
execute
method used here takes a plain
java.util.Map
...
private
val
insertActor = SimpleJdbcInsert(dataSource)
.withTableName(
"t_actor"
).usingGeneratedKeyColumns(
"id"
...
to actor.lastName)
val
newId = insertActor.executeAndReturnKey(parameters);
return
actor.copy(id = newId.toLong())
}
// ... additional methods
}
Copied!
...
: Actor {
val
parameters = BeanPropertySqlParameterSource(actor)
val
newId = insertActor.executeAndReturnKey(parameters)
return
actor.copy(id = newId.toLong())
...
The
SimpleJdbcCall
class uses metadata in the database to look up names of
in
and
out
...
that returns only scalar values in
VARCHAR
and
DATE
...
INTO
out_first_name, out_last_name, out_birth_date
FROM
t_actor
where
id
...
id
of the actor that you are looking up. The
out
parameters return the data read from the table.
You can declare
SimpleJdbcCall
in a manner similar to declaring
SimpleJdbcInsert
. You
...
Actor();
actor.setId(id);
actor.setFirstName((String) out.get(
"out_first_name"
));
actor.setLastName((String) out.get(
"out_last_name"
));
actor.setBirthDate((Date) out.get(
"out_birth_date"
));
return
actor;
...
private
val
procReadActor = SimpleJdbcCall(dataSource)
.withProcedureName(
"read_actor"
)
fun
...
: Actor {
val
source = MapSqlParameterSource().addValue(
"in_id"
, id)
val
output = procReadActor.execute(source)
return
Actor(
id,
output[
...
The
execute
method takes the IN parameters and returns a
Map
that contains any
out
...
data retrieved. Again, it is important to use the names of the
out
parameters as they
are declared in the stored procedure. Also, the case in the names of the
out
parameters stored in the results map matches that of the
out
parameter names in the
database, which could vary between databases. To make your code more portable, you should
do a case-insensitive lookup or instruct Spring to use a
...
property to
true
. Then you can pass this customized
JdbcTemplate
instance into
...
private
var
procReadActor = SimpleJdbcCall(JdbcTemplate(dataSource).apply {
...
the
declareParameters
method, which takes a variable number of
SqlParameter
objects
as input. See the
next section
...
stored procedure call. There is also an
SqlInOutParameter
for
InOut
parameters
...
SqlInOutParameter
are used to
provide input values. This is different from the
StoredProcedure
class, which (for
...
numeric data or a type name for custom database types. For
out
parameters, you can
provide a
RowMapper
to handle mapping of rows returned from a
REF
cursor. Another
option is to specify an
SqlReturnType
...
(named
executeObject
) is also available for stored procedures that have only one
out
...
{
private
SimpleJdbcCall funcGetActorName;
public
void
setDataSource
(DataSource dataSource)
...
"in_id"
, id);
String name = funcGetActorName.executeFunction(String
.
class
,
in
...
private
val
funcGetActorName = SimpleJdbcCall(jdbcTemplate)
.withFunctionName(
"get_actor_name"
)
fun
...
, id)
return
funcGetActorName.executeFunction(String::
class
.
java
,
...
The
executeFunction
method used returns a
String
that contains the return value from the
function call.
Returning a
ResultSet
or REF Cursor from a
SimpleJdbcCall
...
"actors"
,
BeanPropertyRowMapper.newInstance(Actor
.
class
))
;
...
()
{
Map m = procReadAllActors.execute(
new
...
private
val
procReadAllActors = SimpleJdbcCall(JdbcTemplate(dataSource).apply {
isResultsMapCaseInsensitive =
true
}).withProcedureName(
"read_all_actors"
...
: List<Actor> {
val
m = procReadAllActors.execute(mapOf<String, Any>())
return
m[
"actors"
]
