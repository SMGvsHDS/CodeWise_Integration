...
Unified JDBC Query/Update Operations:
JdbcClient
Using
...
RowCallbackHandler
interface extracts values from each row of a
ResultSet
.
You can use
JdbcTemplate
...
The
DataSource
should always be configured as a bean in the Spring IoC container. In
...
```
val
countOfActorsNamedJoe = jdbcTemplate.queryForObject<
Int
>(
"select count(*) from t_actor where first_name = ?"
, arrayOf(
...
new
Actor();
newActor.setFirstName(resultSet.getString(
"first_name"
));
newActor.setLastName(resultSet.getString(
"last_name"
));
return
newActor;
...
new
Actor();
actor.setFirstName(resultSet.getString(
"first_name"
));
actor.setLastName(resultSet.getString(
"last_name"
));
return
actor;
...
sense to remove the duplication present in the two
RowMapper
...
private
final
RowMapper<Actor> actorRowMapper = (resultSet, rowNum) -> {
Actor actor =
new
Actor();
actor.setFirstName(resultSet.getString(
...
```
val
actorMapper = RowMapper<Actor> { rs: ResultSet, rowNum:
Int
->
Actor(rs.getString(
"first_name"
...
return
new
JdbcCorporateEventDao(dataSource);
}
@Bean
(destroyMethod =
"close"
...
class
=
"org.example.jdbc.JdbcCorporateEventDao"
>
<
constructor-arg
ref
...
public
class
JdbcCorporateEventRepositoryConfiguration
{
@Bean
(destroyMethod =
"close"
...
extend from it, your sub-class inherits a
setDataSource(..)
method from the
JdbcDaoSupport
...
If your application accesses multiple databases, you may want multiple
JdbcTemplate
...
section describes only those areas of the
NamedParameterJdbcTemplate
class that differ
from the
JdbcTemplate
...
return
this
.namedParameterJdbcTemplate.queryForObject(sql, namedParameters, Integer
.
class
)
;
...
sql
variable and the corresponding value that is plugged into the
namedParameters
variable (of type
MapSqlParameterSource
).
...
MapSqlParameterSource
class is a
simple implementation that is an adapter around a
java.util.Map
, where the keys
...
) and uses the properties of the wrapped JavaBean as the source
...
"select count(*) from t_actor where first_name = :firstName and last_name = :lastName"
val
namedParameters = BeanPropertySqlParameterSource(exampleActor)
return
namedParameterJdbcTemplate.queryForObject(sql, namedParameters,
Int
::
...
Unified JDBC Query/Update Operations:
JdbcClient
As of 6.1, the named parameter statements of
NamedParameterJdbcTemplate
and the positional
parameter statements of a regular
JdbcTemplate
...
this
.jdbcClient.sql(
"select count(*) from t_actor where first_name = :firstName"
)
.param(
"firstName"
, firstName)
...
themselves.
JdbcClient
is a flexible but simplified facade for JDBC query/update statements.
Advanced capabilities such as batch inserts and stored procedure calls typically require
extra customization: consider Spring’s
SimpleJdbcInsert
and
SimpleJdbcCall
classes or
plain direct
JdbcTemplate
usage for any such capabilities not available in
JdbcClient
.
Using
SQLExceptionTranslator
...
DataAccessException
.
As of 6.0, the default exception translator is
SQLExceptionSubclassTranslator
,
detecting JDBC 4
SQLException
subclasses with a few extra checks, and with a fallback
to
SQLState
...
SQLExceptionTranslator
that is used by default when a file named
sql-error-codes.xml
...
on codes held in a JavaBean type class called
SQLErrorCodes
...
SQLErrorCodeSQLExceptionTranslator
is used, so this rule does not apply. It
...
The
SQLErrorCodesFactory
is used by default to define error codes and custom
exception translations. They are looked up in a file named
sql-error-codes.xml
from the
classpath, and the matching
SQLErrorCodes
instance is located based on the database
...
return
new
DeadlockLoserDataAccessException(task, sqlEx);
}
return
null
;
...
Copied!
```
In the preceding example, the specific error code (
-12345
) is translated while
other errors are left to be translated by the default translator implementation.
To use this custom translator, you must pass it to the
JdbcTemplate
through the
method
...
CustomSQLErrorCodesTranslator tr =
new
CustomSQLErrorCodesTranslator();
...
.
Running Statements
Running an SQL statement requires very little code. You need a
DataSource
and a
JdbcTemplate
...
one row, use
queryForObject(..)
. The latter converts the returned JDBC
Type
to the
...
update. There is no standard single way to create an appropriate
PreparedStatement
...
jdbcTemplate.update({
it.prepareStatement (INSERT_SQL, arrayOf(
"id"
)).apply { setString(
1
, name) }
