...
StoredProcedure
class) can often
be replaced with straight
JdbcTemplate
calls. Often, it is simpler to write a DAO
method that calls a method on a
JdbcTemplate
...
RowMapper
instance that can
create one object per row obtained from iterating over the
ResultSet
that is created
during the execution of the query. The
SqlQuery
...
abstract
mapRow(..)
method to convert each row of the supplied
ResultSet
into an
...
class
ActorMappingQuery
(ds: DataSource) : MappingSqlQuery<Actor>(ds,
"select id, first_name, last_name from t_actor where id = ?"
) {
init
{
...
for this customer query takes a
DataSource
...
```
private
ActorMappingQuery actorMappingQuery;
@Autowired
public
void
setDataSource
...
{
return
actorMappingQuery.findObject(id);
}
Copied!
```
```
private
val
actorMappingQuery = ActorMappingQuery(dataSource)
fun
getActor
(id:
Long
...
Copied!
```
The method in the preceding example retrieves the actor with the
id
...
reusable, and, as with all
RdbmsOperation
classes, an update can have parameters and is
defined in SQL. This class provides a number of
update(..)
...
setDataSource(ds);
setSql(
"update customer set credit_rating = ? where id = ?"
);
declareParameter(
new
SqlParameter(
...
*
@param
id for the Customer to be updated
*
@param
rating the new value for credit rating
*
@return
...
The next example of a simple DAO uses a
StoredProcedure
to call a function
(
sysdate()
...
{
private
GetSysdateProcedure getSysdate;
@Autowired
public
void
init
...
private
val
getSysdate = GetSysdateProcedure(dataSource)
val
sysdate: Date
get
() = getSysdate.execute()
...
class
GetSysdateProcedure
(dataSource: DataSource) : StoredProcedure() {
init
{
setDataSource(dataSource)
isFunction =
...
// the 'sysdate' sproc has no input parameters, so an empty Map is supplied...
val
results = execute(mutableMapOf<String, Any>())
return
results[
"date"
]
...
class
TitlesAndGenresStoredProcedure
(dataSource: DataSource) : StoredProcedure(dataSource, SPROC_NAME) {
companion
object
{
private
...
declareParameter(SqlOutParameter(
"titles"
, OracleTypes.CURSOR, TitleMapper()))
declareParameter(SqlOutParameter(
"genres"
, OracleTypes.CURSOR, GenreMapper()))
compile()
}
fun
execute
...
functionality. The next two examples provide code for the two
RowMapper
implementations.
The
TitleMapper
...
new
Title();
title.setId(rs.getLong(
"id"
));
title.setName(rs.getString(
"name"
));
return
title;
...
definition in the RDBMS, you can code a strongly typed
execute(..)
method that would
delegate to the untyped
execute(Map)
...
declareParameter(
new
SqlParameter(CUTOFF_DATE_PARAM, Types.DATE);
declareParameter(
new
SqlOutParameter(
"titles"
