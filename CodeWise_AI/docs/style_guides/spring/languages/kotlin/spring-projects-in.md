...
Final by Default
By default,
all classes and member functions in Kotlin are
final
.
The
open
modifier on a class is the opposite of Java’s
final
...
Spring beans (such as
@Configuration
annotated classes which by default need to be extended at runtime for technical
reasons) are normally proxied by CGLIB. The workaround is to add an
open
...
kotlin-spring
plugin (a preconfigured version of the
kotlin-allopen
...
@Transactional
@Cacheable
Meta-annotation support means that types annotated with
@Configuration
,
@Controller
,
...
annotations are meta-annotated with
@Component
.
Some use cases involving proxies and automatic generation of final methods by the Kotlin compiler require extra
care. For example, a Kotlin class with properties will generate related
final
getters and setters. In order
to be able to proxy related methods, a type level
@Component
annotation should be preferred to method level
@Bean
in
order to have those methods opened by the
kotlin-spring
plugin. A typical use case is
@Scope
and its popular
@RequestScope
specialization.
...
open
on the classes and their member functions. The samples are written for projects
using the
kotlin-allopen
plugin, since this is the most commonly used setup.
...
kotlin-noarg
plugin if the module uses Spring Data object mappings
(such as MongoDB, Redis, Cassandra, and others).
Injecting Dependencies
Favor constructor injection
Our recommendation is to try to favor constructor injection with
val
read-only (and
non-nullable when possible)
properties
,
...
above.
If you really need to use field injection, you can use the
lateinit var
construct,
as the following example shows:
```
...
@NotNull
public
SampleBean sampleBean$demo_kotlin_internal_test() {
return
new
SampleBean();
}
...
Copied!
```
As a consequence, the related bean name represented as a Kotlin string is
"sampleBean\$demo_kotlin_internal_test"
,
instead of
"sampleBean"
for the regular
...
such bean by name, or add
@JvmName("sampleBean")
to disable name mangling.
Injecting Configuration Properties
In Java, you can inject configuration properties by using annotations (such as
@Value("${property}")
).
However, in Kotlin,
$
...
.
Therefore, if you wish to use the
@Value
...
Copied!
```
You can support components (such as Spring Boot actuators or
@LocalServerPort
) that use
the standard
${…​}
syntax alongside components that use the custom
%{…​}
syntax by
declaring multiple
PropertySourcesPlaceholderConfigurer
beans, as the following example
shows:
```
@Bean
fun
kotlinPropertyConfigurer
()
= PropertySourcesPlaceholderConfigurer().apply {
setPlaceholderPrefix(
"%{"
...
property via a JVM system property (or
via the
SpringProperties
mechanism).
Checked Exceptions
...
@Throws
to specify explicitly the checked exceptions thrown (for example
...
the
value
attribute name, unlike other attributes, and specify it as a
vararg
parameter.
To understand what that means, consider
@RequestMapping
...
@RequestMapping
method
attribute is not specified, all HTTP methods will
be matched, not only the
GET
method.
...
List<Foo>
in Kotlin is conceptually equivalent to
java.util.List<? extends Foo>
because
kotlin.collections.List
is declared as
interface List<out E> : kotlin.collections.Collection<E>
.
This needs to be taken into account by using the
out
...
With JUnit Jupiter (JUnit 5), Kotlin test classes can use the
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
annotation to enable single instantiation of test classes, which allows the use of
@BeforeAll
and
@AfterAll
...
@Test
fun
`should return the result of adding the first number to the second number`
()
{
val
sum = calculator.sum(
...
@Test
fun
`should return the result of subtracting the second number from the first number`
()
{
val
