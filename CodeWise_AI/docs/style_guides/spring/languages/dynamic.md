...
are injected with a reference to the
Messenger
do not know that the underlying
implementation is a Groovy script. The following listing shows the
Messenger
interface:
```
package
...
Copied!
```
The following example defines a class that has a dependency on the
Messenger
interface:
```
package
...
You also need to use a Spring
ApplicationContext
implementation as your IoC
...
The final step in the list in the
preceding section
...
in this next slice of code.) The
System.in.read()
...
Messenger messenger = (Messenger) ctx.getBean(
"messenger"
);
...
(the developer) should make to the
Messenger.groovy
...
Copied!
```
When the program runs, the output before the input pause will be
I Can Do The Frug
.
...
value
=
"Passed straight through to the dynamic-language-backed object"
/>
</
lang
>
...
seen an example
of a Groovy-dynamic-language-backed
...
Copied!
```
The resulting output from running the above program is (unsurprisingly)
10
.
(For more interesting examples, see the dynamic language showcase project for a more
complex example or see the examples
Scenarios
...
set some default property values, or specify a custom
MetaClass
...
GroovyObject
reference. We expect that most people want to set a custom
MetaClass
with this
...
new
DelegatingMetaClass(goo.getMetaClass()) {
public
Object
invokeMethod
...
return
super
.invokeMethod(object, methodName, arguments);
}
};
...
class
=
"example.SimpleMethodTracingCustomizer"
/>
...
or even a full Groovy
CompilerConfiguration
object in the same place as Spring’s
GroovyObjectCustomizer
. Furthermore, you may set a common
GroovyClassLoader
with custom
configuration for your beans at the
ConfigurableApplicationContext.setClassLoader
level;
this also leads to shared
GroovyClassLoader
usage and is therefore recommendable in case of
...
interfaces that are specified in the
script-interfaces
...
```
package
org.springframework.showcase.fortune.web
import
...
script-source
=
"/WEB-INF/groovy/FortuneController.groovy"
>
<
lang:property
name
...
The following example shows a Spring
...
Dynamic Language Support
. See that section
for full details on this support and the
lang
elements.
...
the correct schema so that the tags in the
lang
