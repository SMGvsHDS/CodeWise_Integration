...
To begin, you can create and configure
...
Copied!
```
In the preceding example, you could replace
StandardWebSocketClient
with
SockJsClient
,
since that is also an implementation of
WebSocketClient
. The
SockJsClient
can
...
serialized with the configured
MessageConverter
, as the following example shows:
```
session.send(
...
Copied!
```
You can also subscribe to destinations. The
subscribe
methods require a handler
for messages on the subscription and returns a
Subscription
handle that you can
...
Copied!
```
To enable STOMP heartbeat, you can configure
WebSocketStompClient
with a
TaskScheduler
...
forwarded to the broker. In that case you can configure a
TaskScheduler
...
subscribe are processed. To support this, the
StompSession
offers
...
Alternatively, you can also manually add a receipt header to the
StompHeaders
.
Both send and subscribe return an instance of
Receiptable
that you can use to register for receipt success and failure callbacks.
For this feature, you must configure the client with a
TaskScheduler
...
StompFrameHandler
, which lets
it handle ERROR frames in addition to the
handleException
callback for
exceptions from the handling of messages and
handleTransportError
for
transport-level errors including
...
properties of
WebSocketStompClient
to limit the maximum size of inbound and outbound WebSocket
...
is thrown. The default size limit for inbound messages is
64KB
.
```
WebSocketClient webSocketClient =
...
new
WebSocketStompClient(webSocketClient);
stompClient.setInboundMessageSizeLimit(
64
*
1024
);
// 64KB
stompClient.setOutboundMessageSizeLimit(
64
*
1024
);
