...
the user header on the CONNECT
Message
...
: Message<*> {
val
accessor = MessageHeaderAccessor.getAccessor(message,
StompHeaderAccessor::
class
.
java
...
if
(StompCommand.CONNECT == accessor!!.command) {
// Access authentication header(s) and invoke accessor.setUser(user)
}
return
message
...
you need to ensure that the authentication
ChannelInterceptor
config is ordered
