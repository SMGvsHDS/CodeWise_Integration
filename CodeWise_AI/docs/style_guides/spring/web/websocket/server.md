...
When using the
WebSocketHandler
API directly vs indirectly, for example, through the
STOMP
messaging, the application must synchronize the sending of messages
since the underlying standard WebSocket session (JSR-356) does not allow concurrent
sending. One option is to wrap the
WebSocketSession
with
...
Copied!
```
A more advanced option is to extend the
...
option if it needs to configure a custom
...
base class that you can use to decorate
a
WebSocketHandler
...
or XML namespace. The
ExceptionWebSocketHandlerDecorator
catches all uncaught
exceptions that arise from any
WebSocketHandler
method and closes the WebSocket
session with status
1011
...
For Jakarta WebSocket servers, you can add a
