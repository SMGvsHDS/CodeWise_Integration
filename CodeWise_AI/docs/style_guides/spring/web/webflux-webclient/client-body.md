...
⌘ + k
Request Body
The request body can be encoded from any asynchronous type handled by
...
```
val
personDeferred: Deferred<Person> = ...
client.post()
.uri(
"/persons/{id}"
, id)
...
Copied!
```
Alternatively, if you have the actual value, you can use the
bodyValue
shortcut method,
as the following example shows:
Java
...
```
Form Data
To send form data, you can provide a
MultiValueMap<String, String>
as the body. Note that the
content is automatically set to
...
Copied!
```
You can also supply form data in-line by using
BodyInserters
, as the following example shows:
Java
Kotlin
...
either
Object
instances that represent part content or
HttpEntity
instances that represent the content and
headers for a part.
MultipartBodyBuilder
provides a convenient API to prepare a
multipart request. The following example shows how to create a
MultiValueMap<String, ?>
:
Java
Kotlin
...
type is determined automatically based on the
HttpMessageWriter
...
Once a
MultiValueMap
is prepared, the easiest way to pass it to the
WebClient
is
through the
body
...
represent regular form data (that is,
application/x-www-form-urlencoded
), you need not
set the
Content-Type
...
inline-style, through the built-in
BodyInserters
, as the following example shows:
Java
Kotlin
...
```
PartEvent
To stream multipart data sequentially, you can provide multipart content through
PartEvent
objects.
...
.
You can concatenate the streams returned from methods via
Flux::concat
