...
⌘ + k
Filters
You can register a client filter (
...
.filter { request, next ->
val
filtered = ClientRequest.from(request)
.header(
"foo"
,
"bar"
...
Copied!
```
Filters can be added or removed by mutating an existing
WebClient
instance, resulting
in a new
WebClient
...
its content or to otherwise propagate it downstream to the
WebClient
which will ensure
the same. Below is a filter that handles the
UNAUTHORIZED
...
{
return
(request, next) -> next.exchange(request).flatMap(response -> {
if
...
val
newRequest = ClientRequest.from(request).build()
next.exchange(newRequest)
}
}
else
...
ExchangeFilterFunction
interface to create
a custom filter class that helps with computing a
Content-Length
header for
PUT
and
...
public
class
MultipartExchangeFilterFunction
implements
ExchangeFilterFunction
{
@Override
...
request.body().insert(
new
BufferingDecorator(outputMessage), context)).build()
);
}
else
{
...
{
return
DataBufferUtils.join(body).flatMap(buffer -> {
...
class
BufferingDecorator
(delegate: ClientHttpRequest) : ClientHttpRequestDecorator(delegate) {
override
fun
writeWith
(body:
