...
()
;
List<Person> persons = client.get().uri(
"/persons"
).retrieve()
.bodyToFlux(Person
.
...
Kotlin
```
Mono<Person> personMono = client.get().uri(
"/person/{id}"
, personId)
.retrieve().bodyToMono(Person
.
class
)
;
Mono<List<Hobby>> hobbiesMono = client.get().uri(
"/person/{id}/hobbies"
, personId)
.retrieve().bodyToFlux(Hobby
.
class
).
collectList
...
mapOf(
"person"
to personDeferred.await(),
"hobbies"
to hobbiesDeferred.await())
}
Copied!
```
...
or
Mono
, you should never have to block in a Spring MVC or Spring WebFlux controller.
Simply return the resulting reactive type from the controller method. The same principle apply to
Kotlin Coroutines and Spring WebFlux, just use suspending function or return
Flow
in your
controller method .