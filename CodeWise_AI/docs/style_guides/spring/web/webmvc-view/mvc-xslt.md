...
name of our XSLT view. See
Annotated Controllers
for details of Spring Web MVC’s
Controller
interface. The XSLT controller turns the
...
The controller logic is encapsulated in a
@Controller
class, with the
...
for
(String word : words) {
Element wordNode = document.createElement(
"word"
);
...
words) {
val
wordNode = document.createElement(
"word"
)
val
...
can also load an XML file as a
Resource
...
DOM document into it to generate our view. As shown in the
XsltViewResolver
configuration, XSLT templates live in the
war
file in the
WEB-INF/xsl
directory
