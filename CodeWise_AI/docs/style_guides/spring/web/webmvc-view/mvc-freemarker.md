...
Copied!
```
Alternatively, you can also declare the
FreeMarkerConfigurer
...
can also use nested fields, such as
command.address.street
. The
bind
macro assumes the
default HTML escaping behavior specified by the
ServletContext
parameter
defaultHtmlEscape
in
...
FTL definition
message
(output a string from a resource bundle based on the code parameter)
<@spring.message code/>
messageText
(output a string from a resource bundle based on the code parameter,
...
formInput
(standard input field for gathering user input)
...
formTextarea
(large text field for gathering long, freeform text input)
<@spring.formTextarea path, attributes/>
formSingleSelect
(drop down box of options that let a single required value be
selected)
<@spring.formSingleSelect path, options, attributes/>
formMultiSelect
(a list box of options that let the user select 0 or more values)
<@spring.formMultiSelect path, options, attributes/>
formRadioButtons
(a set of radio buttons that let a single selection be made
from the available choices)
<@spring.formRadioButtons path, options separator, attributes/>
formCheckboxes
(a set of checkboxes that let 0 or more values be selected)
<@spring.formCheckboxes path, options, separator, attributes/>
formCheckbox
(a single checkbox)
<@spring.formCheckbox path, attributes/>
showErrors
(simplify display of validation errors for the bound field)
...
formPasswordInput
are not actually
required, as you can use the normal
formInput
macro, specifying
hidden
or
...
: A
Map
of all the available values that can be selected from in the input
...
controller. You can use any
Map
implementation, depending on required behavior.
For strictly sorted maps, you can use a
SortedMap
(such as a
TreeMap
) with a
...
remains valid until a new bind occurs, so the
showErrors
...
values for the attributes parameter. The following example shows how to use the
formInput
and
showErrors
macros:
...
parameter list. Commonly, the second parameter (
attributes
...
formRadioButtons
formCheckboxes
Each of the four macros accepts a
Map
...
Copied!
```
The preceding listing renders a line of radio buttons, one for each value in
cityMap
, and uses a
separator of
""
...
keys are what the form actually submits as
POST
...
```
protected
Map<String, ?> referenceData(HttpServletRequest request)
throws
Exception {
...
compliant and that use the default value for HTML escaping defined in your
web.xml
file, as
...
true
for a
model or context variable named
xhtmlCompliant
