...
```
GET
/spring-websocket-portfolio/portfolio
HTTP/1.1
Host:
localhost:8080
Upgrade:
...
Server
See equivalent in the Servlet stack
To create a WebSocket server, you can first create a
WebSocketHandler
.
...
return
new
SimpleUrlHandlerMapping(map, order);
}
}
Copied!
```
...
Return a
Mono<Void>
that completes when receiving completes.
```
class
ExampleHandler
:
...
Return a
Mono<Void>
that does not complete while we continue to receive.
```
class
ExampleHandler
:
...
3
Join the streams and return a
Mono<Void>
that completes when either stream ends.
```
class
ExampleHandler
:
...
source: Flux<String> = ...
val
output = session.send(source.map(session::textMessage))
(
2
)
return
...
When running on Netty, applications must use
...
setting a
Predicate<String>
to extract attributes from the
WebSession
and insert them
...
WebSocketSession
handling to take advantage of the Tomcat-specific
API to suspend receiving messages for back pressure.
To start a WebSocket session, you can create an instance of the client and use its
execute
methods:
Java
Kotlin
...
Copied!
```
Some clients, such as Jetty, implement
Lifecycle
