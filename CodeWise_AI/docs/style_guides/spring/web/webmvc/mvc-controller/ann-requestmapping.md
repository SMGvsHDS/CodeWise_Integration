...
AntPathMatcher
. In addition, it also
supports the capturing pattern, for example,
{*spring}
...
Captured URI variables can be accessed with
@PathVariable
. For example:
Java
Kotlin
...
Copied!
```
URI variables are automatically converted to the appropriate type, or
TypeMismatchException
is raised. Simple types (
int
,
long
,
...
DataBinder
.
You can explicitly name URI variables (for example,
@PathVariable("customId")
), but you can
leave that detail out if the names are the same and your code is compiled with the
-parameters
compiler flag.
The syntax
{varName:regex}
...
void
handle
(@PathVariable String name, @PathVariable String version, @PathVariable String ext)
{
// ...
}
Copied!
...
Embedded
${…​}
placeholders that are resolved on startup via
...
one of the following depending on whether use of parsed
PathPattern
...
sorted last. Also, prefix patterns (such as
/public/**
) are considered less
...
.*
suffix pattern
matching where a controller mapped to
/person
is also implicitly mapped to
/person.*
. As a consequence path extensions are no longer used to interpret
the requested content type for the response — for example,
/person.pdf
,
/person.xml
,
and so on.
Using file extensions in this way was necessary when browsers used to send
Accept
headers
...
Having a way to request content types other than through the
"Accept"
header can still
...
them to a list of explicitly registered extensions through the
mediaTypes
property of
ContentNegotiationConfigurer
.
...
See equivalent in the Reactive stack
You can narrow the request mapping based on the
Content-Type
of the request,
...
The
consumes
attribute also supports negation expressions — for example,
!text/plain
means any
content type other than
text/plain
.
You can declare a shared
consumes
attribute at the class level. Unlike most other
request-mapping attributes, however, when used at the class level, a method-level
consumes
attribute
...
presence of a request parameter (
myParam
...
@PostMapping
, and others).
You can explicitly map the
@RequestMapping
method to HTTP HEAD and HTTP OPTIONS, but that
is not necessary in the common case.
Custom Annotations
See equivalent in the Reactive stack
Spring MVC supports the use of
composed annotations
for request mapping. Those are annotations that are themselves meta-annotated with
@RequestMapping
and composed to redeclare a subset (or all) of the
@RequestMapping
...
controller methods should be mapped to a specific HTTP method versus using
@RequestMapping
,
...
method, where
you can check the custom attribute and return your own
RequestCondition
.
...
It is an approach commonly used in Spring Cloud, and it is why
@HttpExchange
is
supported as an alternative to
@RequestMapping
...
For method parameters and returns values, generally,
@HttpExchange
supports a
subset of the method parameters that
@RequestMapping
