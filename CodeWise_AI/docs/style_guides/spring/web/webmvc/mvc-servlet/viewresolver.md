...
Spring MVC defines the
ViewResolver
and
View
interfaces that let you render
models in a browser without tying you to a specific view technology.
ViewResolver
...
The following table provides more details on the
ViewResolver
hierarchy:
...
(in
effect, Servlets and JSPs) and subclasses such as
JstlView
. You can specify the view
...
ViewResolver
interface that resolves a view based on the
...
Implementation of the
ViewResolver
interface that interprets a view name as a
...
Each such
View
can be defined as a bean, for example, in XML or in configuration classes.
Handling
See equivalent in the Reactive stack
...
view could not be found. However, in the case of JSPs and
InternalResourceViewResolver
,
the only way to figure out if a JSP exists is to perform a dispatch through
RequestDispatcher
. Therefore, you must always configure an
InternalResourceViewResolver
to be last in the overall order of view resolvers.
Configuring view resolution is as simple as adding
ViewResolver
beans to your Spring
configuration. The
MVC Config
provides a dedicated configuration API for
View Resolvers
and for adding logic-less
View Controllers
...
by the client. The representation can be determined from the
Accept
header or from a
...
to the client. If a compatible view cannot be supplied by the
ViewResolver
chain,
the list of views specified through the
DefaultViews
property is consulted. This
latter option is appropriate for singleton
Views
that can render an appropriate
representation of the current resource regardless of the logical view name. The
Accept
