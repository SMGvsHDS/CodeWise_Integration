...
> 원문: https://google.github.io/styleguide/shellguide.html
...
Executables must start with#!/bin/bashand minimal flags. Usesetto set
shell options so that calling your script asbash script_namedoes not break
its functionality.


Restricting all executable shell scripts tobashgives us a consistent shell
...
structured languagenow. Bear in mind that scripts grow. Rewrite your
...
then prefer to use a.shextension.
This enables you to use the recommended naming convention, with a source
file likefoo.shand a build rule namedfoo.
- If the executable will be added directly to the user’sPATH, then prefer
...
TODOs should include the stringTODOin all caps, followed by the name,
e-mail address, or other identifier of the person with the best context about
the problem referenced by theTODO. The main purpose is to have a consistentTODOthat can be searched to find out how to get more details upon request. ATODOis not a commitment that the person referenced will fix the problem. Thus
when you create aTODO, it is almost always your name that is given.
...
should be done with ahere documentor an embedded newline if possible.
...
string-matching them (such asgrep) is valuable for maintenance.
...
principles as with braces when declaring functions. That is:; thenand; doshould be on the same line as theif/for/while/until/select.elseshould be on its own line and closing statements (fianddone) should be on
...
The matching expressions are indented one level from thecaseandesac.
...
Simple commands may be put on the same line as the patternand;;as long as the expression remains readable. This is
...
NOTE: Using braces in${var}isnota form of quoting. “Double quotes” must
be usedas well.
...
flags. SeeArraysbelow.
- Optionally quote shell-internal, readonlyspecial variablesthat are defined to be integers:$?,$#,$$,$!. Prefer quoting of
...
TheShellCheck projectidentifies common bugs and
...
should not be used to facilitate more complex data structures (seeWhen to use Shellabove).
...
avoided, as it inevitably leads to authors usingevalor trying to nest quotes inside the string, which does not give
...
avoid confusing quoting issues. Use quoted expansion –"${array[@]}"– to access arrays. However, if more
advanced data manipulation is required, shell scripting should be
avoided altogether; seeabove.


### Pipes to While


Use process substitution or thereadarraybuiltin (bash4+) in preference to
piping towhile. Pipes create a subshell, so any variables modified within a
pipeline do not propagate to the parent shell.


The implicit subshell in a pipe towhilecan introduce subtle bugs that are
hard to track down.
...
redirecting from a subshell to awhilewithout putting thewhile(or any
...
Alternatively, use thereadarraybuiltin to read the file into an array, then
loop over the array’s contents. Notice that (for the same reason as above) you
need to use a process substitution withreadarrayrather than a pipe, but with
...
Note: Be cautious using a for-loop to iterate over output, as infor var in
...
command inside$(...)), awhile readloop orreadarrayis often safer
and clearer.
...
lexicographical comparisons instead; seeTesting Strings).
For preference, don’t use[[ … ]]at allfor numeric comparisons, use(( … ))instead.
...
many times faster thanexpr.


When using variables, the${var}(and$var)
forms are not required within$(( … )). The shell knows
to look upvarfor you, and omitting the${…}leads to cleaner code. This is slightly contrary to
...
Although commonly seen in.bashrcfiles, aliases should be avoided in scripts.
As theBash manualnotes:
...
Parentheses are required after the function name. The keywordfunctionis
...
Thefunctionkeyword is extraneous when “()” is present
...
usinglocalwhen declaring them. This avoids polluting the global namespace
...
assignment value is provided by a command substitution; as thelocalbuiltin does not propagate the exit code from the
command substitution.
...
file. Only includes,setstatements and setting constants
...
A function calledmainis required for scripts long enough
to contain at least one other function.


In order to easily find the start of the program, put the main program in a
function calledmainas the bottom-most function. This provides consistency
with the rest of the code base as well as allowing you to define more variables
aslocal(which can’t be done if the main code is not a function). The last
non-comment line in the file should be a call tomain:
...
Bash also has thePIPESTATUSvariable that allows
...
However, asPIPESTATUSwill be overwritten as soon as you
do any other command, if you need to act differently on errors based
on where it happened in the pipe, you’ll need to assignPIPESTATUSto another variable immediately after running
...
We prefer the use of builtins such as theParameter Expansionfunctionality provided bybashas it’s more efficient, robust, and portable
(especially when compared to things likesed). See also the=~operator.
